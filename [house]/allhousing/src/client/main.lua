...
RegisterNetEvent('Allhousing:client:addGarage')
AddEventHandler('Allhousing:client:addGarage', function(houseid ,slot)
	...
	TriggerServerEvent('bb-garages:server:addHouseGarage', houseid, coords, slot)
      ...
      if Config.Use3DText then
        if closestExtDist < Config.TextDistance3D then
          ...
          if closestExtDist < Config.InteractDistance then
            --DrawText3D(house[closestExt].x,house[closestExt].y,house[closestExt].z, Labels["InteractDrawText"] .."Tương tác") 
	    --ESX.ShowFloatingHelpNotification("~h~Nhà số:[~y~".. house.Id .. "~s~]\n~h~~r~Nhà chưa có ai sở hữu ~r~",vector3(house[closestExt].x,house[closestExt].y,house[closestExt].z))	          
          else
            --DrawText3D(house[closestExt].x,house[closestExt].y,house[closestExt].z, Labels["InteractHelpText"])
            --ESX.ShowFloatingHelpNotification("abcdsa",vector3(house[closestExt].x,house[closestExt].y,house[closestExt].z))
          end
          if house.OwnerName == '' then
	    ESX.ShowFloatingHelpNotification("~h~Nhà số: [~y~".. house.Id .. "~s~]\n~h~~r~Nhà chưa có ai sở hữu ~r~",vector3(house[closestExt].x,house[closestExt].y,house[closestExt].z))	
            --DrawText3D(house[closestExt].x,house[closestExt].y,house[closestExt].z, "~h~Nhà số:[~y~".. house.Id .. "~s~]\n~h~~r~Nhà chưa có ai sở hữu ~r~")
            else 
	    ESX.ShowFloatingHelpNotification("~h~Nhà số: [~y~".. house.Id .. "~s~]\n~h~~r~Nhà của: ~r~"..house.OwnerName.."~s~",vector3(house[closestExt].x,house[closestExt].y,house[closestExt].z))
            --DrawText3D(house[closestExt].x,house[closestExt].y,house[closestExt].z, "~h~Nhà số:[~y~".. house.Id .. "~s~]\n~h~~r~Nhà của: ~r~"..house.OwnerName.."~s~")
          end
        end
      end
