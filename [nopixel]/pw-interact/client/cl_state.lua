...
function hasKeys(pEntity)
	...
	-- end, GetVehicleNumberPlateText(pEntity))
...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('pw-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("pw-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['pw-vehicles']:GetVehicleIdentifier(pEntity)
  local vehicleOwner = NPX.Procedures.execute("pw-ottosauto:getVehicleOwner", vin)
...
RegisterNetEvent("pw-jobmanager:playerBecameJob")
AddEventHandler("pw-jobmanager:playerBecameJob", function(job, name, notify)
...
RegisterNetEvent("pw-police:cuffs:state")
AddEventHandler("pw-police:cuffs:state", function(pIsHandcuffed, pIsHandcuffedAndWalking)
...
RegisterNetEvent('pw-police:drag:escort')
AddEventHandler('pw-police:drag:escort', function()
    isEscorting = true
end)

RegisterNetEvent('pw-police:drag:releaseEscort')
AddEventHandler('pw-police:drag:releaseEscort', function()
    isEscorting = false
end)
...
AddEventHandler("pw-fishing:currentlyRentedBoat", function(handle)
    currentlyRentedBoat = handle
end)

AddEventHandler("pw-interact:openFridge", function()
  TriggerEvent("server-inventory-open", "31", "Craft");
end)

AddEventHandler("pw-interact:grabDrink", function(args, entity, context)
  TriggerEvent("server-inventory-open", "1", context.zones["bar:grabDrink"].name)
end)

RegisterNetEvent('pw-interact:setNetVehicleBypassed', function(netId, bypass)
