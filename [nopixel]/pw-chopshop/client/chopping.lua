...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["pw-taskbar"]:taskBar(14000, "Đang phá rỡ")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    local finished = exports["pw-taskbar"]:taskBar(25000, "Đang phá rỡ")
        ...
        while state.active do
            ...
            if not IsInsideVehicle and distance and distance <= 10.0 then
                ...
                if bone.type == "door" and distance <= 1.6 then
                    local rawText = "Nhấn ~w~~g~[E]~w~ để phá rỡ cửa xe"
                    inDistance, chopText = true, rawText
                elseif bone.type == "tyre" and distance <= 1.6 then
                    local rawText = "Nhấn ~w~~g~[E]~w~ để phá rỡ bánh xe"
                    inDistance, chopText = true, rawText
                elseif bone.type == "remains" and distance <= 1.8 then
                    local rawText = "Nhấn ~w~~g~[E]~w~ để phá rỡ phần còn lại"
                    inDistance, chopText = true, rawText
                end
                ...
                if inDistance then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        if success then
                            RPC.execute("pw-chopshop:chopTheVehicle", netId, vehicleModel, boneType)

                            if not exports["pw-flags"]:HasVehicleFlag(vehicle, "isScrapVehicle") then
                                exports["pw-flags"]:SetVehicleFlag(vehicle, "isScrapVehicle", true)
                            end
                        end
                    end
                end
...
--[[ RegisterCommand('chop', function ()
    local vehicle = exports['pw-interact']:GetCurrentEntity()
...
RegisterNetEvent('pw-chopshop:ChopChop')
AddEventHandler('pw-chopshop:ChopChop', function()
    local vehicle = exports['pw-interact']:GetCurrentEntity()
