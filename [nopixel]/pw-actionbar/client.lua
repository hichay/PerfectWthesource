...
AddEventHandler('pw-items:SetAmmo', function(sentammoTable)
	...
	while true do
      ...
      if weapon == "-37975472" then
        ...
        lastEquippedItemToRemove = nil
			...
			if throwableWeapons[weapon] and exports["pw-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["pw-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler('actionbar:ammo', function(hash,amount,addition)
	...
	if ammoTable == nil then
		 TriggerServerEvent("pw-weapons:getAmmo")
		Citizen.Wait(2000)
	end
...
function updateAmmoTable(newammo,ammoType)
	...
	TriggerServerEvent("pw-weapons:updateAmmo",newammo,ammoType,ammoTable)
end
...
RegisterNetEvent('pw-item:CheckClientAmmo')
AddEventHandler('pw-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["pw-inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["pw-inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["pw-inventory"]:hasEnoughOfItem("extended_ap",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_APPISTOL_CLIP_02` )	
	end

	if exports["pw-inventory"]:hasEnoughOfItem("extended_sns",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_SNSPISTOL_CLIP_02` )	
	end
	
	if exports["pw-inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["pw-inventory"]:hasEnoughOfItem("extended_micro",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_MICROSMG_CLIP_02` )	
	end

	if exports["pw-inventory"]:hasEnoughOfItem("weapon_scope", 1, false, true) then
	...
	if exports["pw-inventory"]:hasEnoughOfItem("TinyScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_MACRO` )	
	end

	if exports["pw-inventory"]:hasEnoughOfItem("extended_tec9",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_MACHINEPISTOL_CLIP_02` )	
	end

	if exports["pw-inventory"]:hasEnoughOfItem("huntingscope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_LARGE` )	
	end
	if exports["pw-inventory"]:hasEnoughOfItem("weapon_uzi_extended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["pw-inventory"]:hasEnoughOfItem("weapon_uzi_foldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	
	if exports["pw-inventory"]:hasEnoughOfItem("weapon_uzi_woodstock", 1, false, true) then
