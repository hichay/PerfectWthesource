...
RegisterCommand("adminmenu", function(source, args)
    local src = source
    local steamIdentifier = ESX.GetPlayerFromId(source).identifier
	print(steamIdentifier)
    MySQL.single('SELECT `group` FROM users WHERE `identifier`= ?', {steamIdentifier}, function(data)
		print(data.group)
        if data.group == "dev" or data.group == "admin" or data.group == "mod" then
			print('ok')
...
RPC.register('pw-adminmenu:server-getplayerdata', function(source, cb, License)
    TriggerClientEvent('table',-1,License)
    for LicenseId, _ in pairs(License) do
        print(LicenseId)
        print(_)
        local xPlayer = ESX.GetPlayerFromIdentifier(LicenseId)
        local PlayerCharInfo = {
            Name = GetPlayerName(xPlayer),
            Steam = xPlayer.identifier,
            CharName = xPlayer.getName(),
            Source = xPlayer.source,
            CitizenId = xPlayer.id
        }
        cb(PlayerCharInfo)
    end
end)
...
RegisterNetEvent("varial-admin/server/toggle-blips", function()
    ...
    for _, xPlayer in pairs(ESX.GetExtendedPlayers()) do
        local NewPlayer = {
            ServerId = xPlayer,
            Name = xPlayer.getName(),
            Coords = GetEntityCoords(GetPlayerPed(xPlayer)),
        }
...
RegisterNetEvent("Admin:ServerRevive", function(ServerId)
    local src = source
...
AddEventHandler("varial-adminmenu:CheckInventory", function(target)
    ...
    print(target)
    if target ~= "" then
        local xPlayer = ESX.GetPlayerFromIdCard(target)

        Wait(1)
        TriggerClientEvent("server-inventory-open", src, "1", ""..xPlayer.identifier)
    else
