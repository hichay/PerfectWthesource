            ...
            for vehicle, enabled in pairs(Checking) do
                ...
                if inside and not Ghosted[vehicle] then
                    ...
                    if Config.antiGhostYeetVehicles then
                        TriggerEvent("pw-density:yeet", vehicle)
                    end
...
AddEventHandler("pw-density:yeet", function (vehicle)
    local vin = exports["pw-vehicles"]:GetVehicleIdentifier(vehicle)

    if vin then return end

    exports["pw-sync"]:SyncedExecution("DeleteEntity", vehicle)
end)
...
AddEventHandler("pw-config:configLoaded", function (pId)
    if (pId ~= "pw-density") then return end
