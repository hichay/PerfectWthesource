...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback('rl-jewellery:gettimeoutstatus', function(rs)
        isInTimeout = rs
    end)
    isLoggedIn = true
end)
                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    ...
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        ...
                                        ESX.TriggerServerCallback('rl-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                if validWeapon() then
                                                    smashVitrine(case)
                                                else                                                 
													TriggerEvent("DoLongHudText","Your weapon doesn\'t seem strong enough")
                                                end
                                            else
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
		...
		TriggerEvent("DoLongHudText","You broke the glass")  
    end

    smashing = true

    ESX.Progressbar("smash_vitrine", "Robbing", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
