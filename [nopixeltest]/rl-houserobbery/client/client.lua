...
AddEventHandler("esx:setJob", function(job)
 ...
 RegisterCommand('breach', function(source, args)  
  ...
  if PlayerData.job ~= nil and PlayerData.job.name == 'police' then 
   ...
   if not found then   
	TriggerEvent("DoLongHudText","No house")
   end  
  else  
   TriggerEvent("DoLongHudText","You are not police")
  end 
 end)
...
AddEventHandler('lockpicks:UseLockpick', function(advanced)
  ...
  if isRobbing and DoesEntityExist(safe) then
    ...
    if GetDistanceBetweenCoords(playerCoords, safepos.x, safepos.y, safepos.z, true) <= 3.0 then
      TriggerEvent("safecrackinghouse:loop",5)
 ...
 if isNight() and not isRobbing then
  ESX.TriggerServerCallback("houserob:checkcops", function(copsNum)
    if copsNum >= Config.MinCops then
      ...
      for id,v in pairs(Config.RobbableHouses) do
        if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
          ...
          ESX.Progressbar("lockpicking_property", "Lockpicking Property", 12000, false, true, {}, {}, {}, {}, function()
            isLockpicking = false
            pedSpawned = false
  ...
  if not isNight() and not isRobbing then
	TriggerEvent("DoLongHudText","Its too bright out")
  end
end)
...
AddEventHandler('houseRobberies:createHouse', function(id)
 ...
 randomAI({x = house.x, y = house.y, z = house.z})
...
function randomAI(generator)
 ...
 if airoll == 1 then
  ...
  TaskPlayAnim(robberyped, dict, 'f_sleep_r_loop_bighouse', 8.0, 8.0, -1, 2, false,false,false)
  SetBlockingOfNonTemporaryEvents(robberyped, true)
  TaskSetBlockingOfNonTemporaryEvents(robberyped, true)
  pedSpawned = true
   ...
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     ...
     if IsControlJustReleased(1, 38) then
	  ...
	  LockpickDoorAnim(20000)
      ESX.Progressbar("search_"..myRobbableItems[i]['name'], "Searching "..myRobbableItems[i]['name'], 20000, false, true, {
...
AddEventHandler('remove:item', function()
