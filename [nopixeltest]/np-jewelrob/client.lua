    ...
    while true do 
        ...
        if distance < 3.0 and not hasrobbed[1] and not hasrobbed[5] and not hasrobbed[10] and not hasrobbed[15] and not hasrobbed[20] then
        	Wait(1)
             ...
             if IsControlJustReleased(0,38) and distance < 1.0 and not robbing then
             	if exports["pw-inventory"]:hasEnoughOfItem("Gruppe6Card3",1,false) then
...
function giveitems()
	if weaponTypeC() > 2 then
		...
		if math.random(1) == 1 then
			TriggerEvent("player:receiveItem", "goldbar",math.random(1))
		end

		if math.random(69) == 69 then
			TriggerEvent("player:receiveItem", "valuablegoods",math.random(1,20))
		end
...
function AttackGlass(num)
	...
	if math.random(100) > 1 or weaponTypeC() > 2 then
		Citizen.Wait(1500)
...
AddEventHandler('event:control:jewelRob', function(useID)
	if not IsPedRunning(PlayerPedId()) and not IsPedSprinting(PlayerPedId()) and not isCop and copAmount < 2 and not hasrobbed[useID] then
		local v = Config.locations[useID]
			...
			for i=1,#Config.locations do
				...
				if (#(GetEntityCoords(PlayerPedId()) - vector3(v[1],v[2],v[3])) < 0.8 ) then
					if (not hasrobbed[i]) then
						...
						if IsControlPressed(0,38) and not robbing and not hasrobbed[i] then
							robbing = true
