...
storePedLocations = {
	...
	[19] =  { ['x'] = -622.3899, ['y'] = -229.7485, ['z'] = 38.109573, ['h'] = 134.02317, ['info'] = ' Jewel robbery 19', ["recent"] = false, ["safe"] = true },
}
...
JewelLocation = {
	[19] =  {  ['x'] = -622.351, ['y'] = -229.8598, ['z'] = 38.109577, ['h'] = 313.56469 , ['info'] = ' Register 1' },
}
...
RegisterNetEvent("store:dosafejewel")
AddEventHandler("store:dosafejewel", function()
	-- if CopsOnline >= 1 then
		...
		TriggerEvent("safecracking:loop",1,"robbery:safe")
	-- else
...
AddEventHandler("inv:advancedLockpick", function()
	...
	if storeid ~= 0 then
		...
		local jewelCoords = JewelLocation[storeid]

		if #(vector3(jewelCoords["x"],jewelCoords["y"],jewelCoords["z"]) - GetEntityCoords(PlayerPedId())) < 3.0 then
			TriggerServerEvent("store:robbery:jewel",storeid)
			robbing = true
			return
		end 
...
AddEventHandler("robbery:safe", function()
	...
	TriggerEvent("player:receiveItem","rollcash",10)
	end
end)
