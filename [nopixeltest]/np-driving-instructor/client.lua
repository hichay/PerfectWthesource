...
RegisterCommand('kiemtra', function()
	TriggerEvent("drivingInstructor:testToggle")
		
end)

RegisterCommand('guiketqua', function()
	TriggerEvent("drivingInstructor:submitTest")
...
function updateInstructor()
  if isInstructorMode then
    checkForVehicle()
    TriggerEvent("DoShortHudText","Chế độ hướng dẫn: Mở")
  else
    TriggerEvent("DoShortHudText","Chế độ hướng dẫn: Tắt")
  end
    ...
    while isInstructorMode do
      ...
      if isInVehicle then
        ...
        if playerVeh ~= 0 and isVehicleAllowed(playerVeh) then -- Check if we weren't in a vehicle before but now are
          if true then -- Only run on vehicles we have keys for
            isInVehicle = true
  ...
  for i = 1, #drivingSchools do
    local drivingSchool = drivingSchools[i]
		...
		if(distance < 3.0) then
			DrawText3Ds(drivingSchool["x"], drivingSchool["y"], drivingSchool["z"], "[Y] Để bắt đầu mở chế độ" )
		end
		 if(distance < 3.0) then
			 return true
		 end
...
AddEventHandler("drivingInstructor:testToggle", function()
  if not isInstructorMode then
    if usingClipboard then -- Close clipboard if we had it open
      ...
      TriggerEvent('DoLongHudText',"Phải đang bật chế độ hướng dẫn!", 2)
    end
  else
...
AddEventHandler("drivingInstructor:submitTest", function()
  ...
  if curTest.cid <= 0  then
    TriggerEvent('DoLongHudText',"Phải điền id người đang thi!", 2)
    return
  end
...
AddEventHandler("drivingInstructor:clipboard", function(isWriting)
  ...
  if usingClipboard then
    ...
    while usingClipboard do
      ...
      if true and not IsEntityPlayingAnim(ped, anim, "base", 3) then
        TaskPlayAnim(ped, anim, "base", 2.0, 3.0, -1, 49, 0, 0, 0, 0)
      end
      Citizen.Wait(1)
    end
