$(document).ready(function () {
  ...
  function setData(data, readOnly) {
    curTest = Object.assign({ cid: 0, instructor: '', points: 20, passed: true, results: {} }, data);
    curTest.points = 20;
  ...
  function closeGui() {
    if (readOnly) {
      // Don't record data
      $.post('http://np-driving-instructor/close', JSON.stringify({ cid: -1, points: 20, passed: true, results: {} }));
    } else {
      curTest.results = {};
  ...
  function calculatePoints() {
    curTest.points = 20; // Reset points to calculate off of
    $('.input__group input').each(function () {
      const _self = $(this);
      const name = _self.prop('name');            
      if (curTest.results[name]) {
