...
AddEventHandler('payment:chopshopscrap', function(rarity,class,playsound)
	
	
		
	if (class == 0 or class == 8) then
		TriggerServerEvent('chopchop:giveitem', 'ClassA')
	elseif (class == 1 or class == 1) then
		TriggerServerEvent('chopchop:giveitem', 'ClassB')
	elseif (class == 4 or class == 9 or class == 18) then
		TriggerServerEvent('chopchop:giveitem', 'ClassC')
	elseif (class == 2 or class == 12) then
		TriggerServerEvent('chopchop:giveitem', 'ClassD')
	elseif (class == 5 or class == 6 or class == 7) then
		TriggerServerEvent('chopchop:giveitem', 'ClassS')
	else
		class = "All"
	end	
	...
	if finished == 100 then
		...
		if not IsPedInVehicle(PlayerPedId(),vehicle,false) then
			if vehicleDifference <= 1 then
				...
				TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 2.0, 'impactdrill', 0.5)
				TriggerServerEvent("chopshop:removevehicle",vehResponse,plate,class)
				Citizen.Wait(2500)
				TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 2.0, 'impactdrill', 0.5)
	    ...
	    if dropOff2 < 20.0 then
	    	...
	    	if not isInVehicle then
	    		...
	    		if DoesEntityExist(veh) and vehResponse > 0 then
	    			...
	    			DrawText3Ds(vehLoc["x"],vehLoc["y"],vehLoc["z"], "[E] Phân rã xe") 
    				if IsControlJustReleased(0,38) then
    				...
    				if IsControlJustReleased(0,38) then
    					...
    					local message = "Danh sách:<br>"
						for i = 1, #currentVehicleList do 
							if not currentVehicleList[i]["resolved"] then
								if currentVehicleList[i]["rarity"] > 10 then
     								if i == 1 then
    									message = message .. " - " .. vehicleList[currentVehicleList[i]["id"]]["name"] .. " (priority) </br>"
    								else
    									message = message .. " - " .. vehicleList[currentVehicleList[i]["id"]]["name"] .. " (priority)</br>"
    								end
								else
    								if i == 1 then
    									message = message .. " - " .. vehicleList[currentVehicleList[i]["id"]]["name"] .."</br>"
    								else
    									message = message .. " - " .. vehicleList[currentVehicleList[i]["id"]]["name"] .."</br>"
    								end
								end
						...
						TriggerServerEvent('qb-phone:server:sendNewMail', {
							sender = "Bãi phế liệu",
							subject = "Thông báo tìm xe!",
							message = message,
							button = {}
						})
						--print('message')
