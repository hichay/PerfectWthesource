...
AddEventHandler('cameraSystem:client:startHacking', function(looper) --Calls this event when you want to start hacking.
    ...
    elseif(Config.HackLocations[looper].type == "mhacking")then
        DoMHacking(looper)
    elseif(Config.HackLocations[looper].type == "fallout")then	
		DoFalloutHacking(looper)
    end
...
function DoMHacking(looper) --Starts the Mhacking sequence minigame. Download the script if you don't have it.
    ...
    TriggerEvent("mhacking:start", math.random(5, 9), math.random(10, 15), OnMHackingDone)

end

function DoFalloutHacking(looper) --Starts the Mhacking sequence minigame. Download the script if you don't have it.
    DoFalloutHacking = looper
    TaskStartScenarioInPlace(globalPed, "PROP_HUMAN_ATM", 0, true)
    Citizen.Wait(2000)
    local finished = exports["fallouthack"]:FalloutHack()
	if finished == 100 then
	   TriggerServerEvent("cameraSystem:server:updateState", Config.HackLocations[DoFalloutHacking].cameras, false)
	   ClearPedTasks(globalPed)
	else
	   TriggerServerEvent('cameraSystem:hackfail', exports['cd_dispatch']:GetPlayerInfo())
	   TriggerServerEvent("cameraSystem:server:updateState", Config.HackLocations[DoFalloutHacking].cameras, true)
	   ClearPedTasks(globalPed)
	end
