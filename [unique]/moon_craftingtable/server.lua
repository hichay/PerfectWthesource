		...
		if Config["craft_enable_fail"] and data.fail_chance then
			...
			if rand <= f then
				...
				TriggerClientEvent('audio:unsuccess',source)
								sendDiscord('Crafting', '**Name: **'.. plyName .. '** คราฟไอเท็มไม่สำเร็จ **')
									xPlayer.addInventoryItem('token_fail', 1)
									--TriggerClientEvent("discordbot:crafting_cl", _source, item.label, amount, false)
			end
		end
		...
		if good then
			...
			TriggerClientEvent("pNotify:SendNotification", _source, {
				...
				layout = "centerRight"
					...
					sendDiscord('Crafting', '**Name: **'.. plyName .. '** | คราฟไอเท็มสำเร็จ: **'  .. itemLabel(data.item, xPlayer.inventory))
TriggerClientEvent('audio:success',source)
--TriggerClientEvent("discordbot:crafting_cl", _source, item.label, amount, true)
