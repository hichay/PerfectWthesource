...
RegisterNetEvent("audio:success")
AddEventHandler(
    "audio:success",
    function()
...
RegisterNetEvent("audio:unsuccess")
AddEventHandler(
    "audio:unsuccess",
    function()
        --Trigger the interaction sound
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "Crash", 0.4)
...
function CraftItem(cate_id, item_id, amount)
    ...
    if
        Config["category"][cate_id] == nil or Config["category"][cate_id]["list"] == nil or
            Config["category"][cate_id]["list"][item_id] == nil or
    ...
    if data.cost ~= nil and data.cost > 0 or Config["craft_cost"] ~= nil and Config["craft_cost"] > 0 then
        local cost = data.cost or Config["craft_cost"]
        if type(cost) ~= "number" then
            SendNUIMessage(
                {
                    type = "dialog",
                    title = "ไฟล์ตั้งค่าผิดพลาด",
                    message = "ติดต่อผู้ดูแลระบบเพื่อแก้ไขปัญหา"
                }
            )
            return
        end
    ...
    if data.blueprint then
        local num = table.Count(data.blueprint)
        for k, v in pairs(inv) do
            if data.blueprint[v.name] and v.count <= 0 then
                SendNUIMessage(
                    {
                        type = "dialog",
...
function GetNearestTable()
    ...
    for k, v in ipairs(Config["craft_table"]) do
        if v.position ~= nil then
            local dist =
                GetDistanceBetweenCoords(v.position["x"], v.position["y"], v.position["z"], pos.x, pos.y, pos.z, true)
        ...
        function()
            ...
            Citizen.CreateThread(
                function()
                    ...
                    local text =
                        string.format("{ x = %.2f, y = %.2f, z = %.2f, h = %.2f }", pos.x, pos.y, pos.z, heading)
                    ...
                    if v.position ~= nil then
                        ...
                        if v.ent == nil then
                            if ESX then
								...
								if not v.disable_model then 
									Citizen.CreateThread(
										function()
											local bench = v.model or GetHashKey("gr_prop_gr_bench_04b") 
												if type(bench) == "string" then 
												bench = GetHashKey(bench) 
												end 
											RequestModel(bench) 
											while not HasModelLoaded(bench) do
												Citizen.Wait(1)
						...
						if v.marker and dist <= (v.market_distance or 10) then 
							local makercolor = v.marker_color or {255, 230, 0}
							local size = v.marker_scale or {2, 2, 2}
							DrawMarker(1, v.position["x"], v.position["y"], v.position["z"], 0, 0, 0, 0, 0, 0, size[1], size[2], size[3], makercolor[1], makercolor[2], makercolor[3], 100, false, true, 2, false, false, false, false) 
						end	
                        ...
                        if dist <= (v.max_distance or 2.0) and craft_percent == -1 and not menu_active then
                            ESX.ShowHelpNotification('Nhấn ~INPUT_CONTEXT~ để mở ~y~Chế tạo~y~')

                            if IsControlJustPressed(0, 38) then
								local kq = true 
								time = GetGameTimer() + 1000
								if v.job and ESX.PlayerData.job and v.job ~= ESX.PlayerData.job.name then 
									local text = Config["translate"]["job_"..v.job] or v.job or "Untimeown"
									kq = false 
									local pos = v.position 
									Citizen.CreateThread(function() 
										while time > GetGameTimer() do 
											Citizen.Wait(0) 
											draw.Simple3DText(pos["x"], pos["y"], pos["z"] + 1.25, Config["translate"]["job_limit"] and string.format(Config["translate"]["job_limit"], text) or "Bạn không thể sử dụng bàn "..text.." nhận", 1) 
										end 
									end) 
								end	
                                -- OpenCraftMenu()
								if kq then 
									print(v.number)
									print(v.type)
									TriggerEvent("server-inventory-open", ""..v.number, ""..v.type);
...
function Holograms()
		while true do
			Citizen.Wait(0)			
				--Hologram No. 1
		if GetDistanceBetweenCoords( 222.56, -807.36, 30.64, GetEntityCoords(GetPlayerPed(-1))) < 5.0 then
			Draw3DText( 222.56, -807.36, 30.64  -1.400, "WipWup 4.0 Craft", 4, 0.1, 0.1)
			--Draw3DText( 222.56, -807.36, 30.64  -1.600, "⚒", 4, 0.1, 0.1)
			--Draw3DText( 222.56, -807.36, 30.64  -1.800, "By teamDvm", 4, 0.1, 0.1)		
		end	
	end
end
]]
