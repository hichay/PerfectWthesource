                        ...
                        if GetDistanceBetweenCoords(s.coords, GetEntityCoords(playerPed)) < 1 then
                            if job == Config.JobRequired and grade >= Config.JobGradeRequired then
                                ...
                                if IsControlJustReleased(0, Keys[Config.PickupEvidenceKey]) then
									if exports["pw-inventory"]:hasEnoughOfItem("evidencebag",1,false) then
										if #evidence < 3 then
											local dict, anim =
											...
											TriggerServerEvent("core_evidence:removeBlood", t)
											TriggerEvent("inventory:removeItem", "evidencebag", 1)
											TriggerEvent("player:receiveItem","sealedevidencebag",1)
											SendTextMessage(
										...
										SendTextMessage("Không có túi đựng bằng chứng")
									end		
                                end
                            else
            ...
            if job == Config.JobRequired and grade >= Config.JobGradeRequired then
                ...
                elseif GetDistanceBetweenCoords(coords, Config.EvidenceAlanysisLocation) < 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) and not analyzing and not analyzingDone then
                        if #evidence > 0 then
							TriggerEvent("inventory:removeItem", "sealedevidencebag", 1)
                            Citizen.CreateThread(
                                function()
...
AddEventHandler("core_evidence:addFingerPrint",function(report)
        Citizen.CreateThread(function()
				...
				if exports["pw-inventory"]:hasEnoughOfItem("evidencebag",1,false) then 
					...
					SendTextMessage("Không có túi đựng")
				end
            end
        )
    end)
...
AddEventHandler("core_evidence:SendTextMessage",
    function(msg)
        SendTextMessage(msg)
    end
)
            ...
            --if IsShockingEventInSphere(102, 235.497, 2894.511, 43.339, 999999.0) then
                if HasEntityBeenDamagedByAnyPed(ped) then
                    ...
                    if Config.ShowBloodSplatsOnGround then
                        local stain = CreateObject(GetHashKey("p_bloodsplat_s"),
                            coords[1],
                            coords[2],
                            coords[3] - 2.0,
                            true,
...
function DrawText3D(x, y, z, text)
    ...
    if onScreen then
        ...
        BeginTextCommandWidth("CUSTOM_TEXT")
