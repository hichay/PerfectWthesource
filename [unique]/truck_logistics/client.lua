...
RegisterNUICallback('repairTruck', function(data, cb)
	if truck and not rentTruck then
		TriggerEvent("ESX:Notify",Lang[Config.lang]['store_truck'],'success')
		return
	end
...
RegisterNUICallback('sellTruck', function(data, cb)
	if truck and not rentTruck then
		TriggerEvent("ESX:Notify",Lang[Config.lang]['store_truck_2'],'success')
		return
	end
...
AddEventHandler('truck_logistics:spawnTruck', function(truck_data)
	if truck then
		TriggerEvent("ESX:Notify",Lang[Config.lang]['already_has_truck'],'error')
		return
	end
	...
	while i > 0 do
		...
		if checkPos == false then
			if i <= 1 then
				TriggerEvent("ESX:Notify",Lang[Config.lang]['occupied_places'],'error')
				return
			end
		else
			break
	...
	TriggerEvent("ESX:Notify",Lang[Config.lang]['already_is_in_garage'],"success")
...
AddEventHandler('truck_logistics:startContract', function(data,job_distance,reward,truck_data)
	if trailer or rentTruck then
		TriggerEvent("ESX:Notify",Lang[Config.lang]['already_has_cargo'],"success")
		return
	end
	if truck and data.contract_type == 0 then
		TriggerEvent("ESX:Notify",Lang[Config.lang]['must_store_truck'],"success")
		return
	end
	...
	if data.contract_type == 0 then
		...
		while i > 0 do
			...
			if checkPos == false then
				if i <= 1 then
					TriggerEvent("ESX:Notify",Lang[Config.lang]['occupied_places'],"success")
					return
				end
			else
				break
	...
	TriggerEvent("ESX:Notify",Lang[Config.lang]['started_job'],"success")
	...
	while trailer do 
		...
		if distance <= 50.0 then
			...
			if distance <= 4.0 and veh == truck and GetEntityHeading(truck) + 10 >= h and GetEntityHeading(truck) - 10 <= h and GetEntityHeading(trailer) + 10 >= h and GetEntityHeading(trailer) - 10 <= h and IsEntityAttachedToEntity(truck,trailer) then
				DrawMarker(22,x,y,z-0.6,0,10.0,0,90.0,h,0.0,3.0,1.0,10.0,0,255,0,50,0,0,0,0)
				...
				DrawMarker(22,x,y,z-0.6,0,10.0,0,90.0,h,0.0,3.0,1.0,10.0,255,0,0,50,0,0,0,0)
			end
		end
		...
		if bodyhealth <= 150 then
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			TriggerEvent("ESX:Notify",Lang[Config.lang]['failed'],"success")
		...
		if IsControlPressed(0,Config.contratos['cancel_contrato']) then
			...
			route_blip = nil
			rentTruck = false
			TriggerEvent("ESX:Notify","Đơn hàng huỷ thành công","success")
			if data.contract_type == 0 then
				DeleteVehicle(truck)
				RemoveBlip(truck_blip)
				truck = nil
...
function spawnVehicle(name,x,y,z,h,vehbody,vehengine,vehtransmission,vehwheels,blip_sprite,blip_color,blip_name)
	...
	if HasModelLoaded(mhash) then
		...
		TriggerEvent("vehiclekeys:client:SetOwner", ESX.Game.GetVehicleProperties(nveh).plate)
