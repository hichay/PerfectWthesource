		...
		for k,v in pairs(Config.market_locations) do
			...
			if not menuactive and distance <= 20.0 then
				...
				if distance <= 2.0 then
					...
					ESX.ShowFloatingHelpNotification("Nhấn ~INPUT_PICKUP~ để mở",vector3(x,y,z))
...
RegisterNUICallback('startJob', function(data, cb)
	...
	TriggerServerEvent('vrp_stores:startContract',empresaAtual,data.item_id)
end)
...
AddEventHandler('vrp_stores:startContract', function(item,truck_level,query_delivery)
	local key = empresaAtual
	job_data[key] = nil
		...
		elseif fase_coleta == 3 then
			...
			if distance <= 50 and veh == truck then
				...
				if distance <= 4 then
					...
					truck_blip = nil
					route_blip = nil
					TriggerServerEvent("vrp_stores:finishContract",key,item,truck_level,distance_traveled,query_delivery)
					return
				end
			end
		end
