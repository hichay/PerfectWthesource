...
window.oncontextmenu = function (event) {
  ...
  if (element != null) {
    ...
    if (columnMenu) {
      ...
      if (acceptedCall == '') {
        html = html + '    <a onclick="acceptCall()">Chấp nhận yêu cầu</a>';
        if (jobInfo[job].canRemoveCall) {
          html = html + '    <a onclick="removeCall()">Xóa yêu cầu</a>';
        }
        if ($(element).hasClass('message')) {
          html = html + '    <a onclick="copyNumber()">Copy số điện thoại</a>';
        }

      }

      if (acceptedCall != '' && acceptedCall == $(element).attr('id')) {
        html = html + '    <a onclick="dismissCall()">Bỏ qua yêu cầu</a>';
      }


      for (const [key, value] of Object.entries(jobInfo)) {
        if (key != job && jobInfo[job].forwardCall) {
          html = html + '    <a onclick="forwardCall(' + value.column + ')">Chuyển tiếp ' + value.label + '</a>';
        }
      }
