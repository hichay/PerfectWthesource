...
function functionRemoveAny(player, itemidsent, removeamount, openedInv) {
  ...
  let string = `UPDATE user_inventory2 SET amount=amount- '${removeamount}' WHERE name='${playerinvname}' and item_id='${itemidsent}'`
...
onNet("server-request-update", async (player) => {
  ...
  let string = `SELECT  amount, id, item_id, name, information, slot, dropped FROM user_inventory2 WHERE name = '${player}' group by item_id`;
...
onNet("server-request-update-src", async (player, src) => {
  ...
  let playerinvname = player
   /* print('and4')  */
  let string = `SELECT  amount, item_id, id, name, information, slot, dropped, creationDate, MIN(creationDate) as creationDate FROM user_inventory2 WHERE name = '${playerinvname}' group by item_id`; // slot
  exports.ghmattimysql.execute(string, {}, function(inventory) {
    /* var ammount = "0"
    for (let i = 0; i < inventory.length; i++) {
    ammount = inventory[i].amount
    }
    print(ammount)
    if (ammount == "0")
    {
          ...
          /* if (itemdata == itemdata.fake) {
            ...
            exports.ghmattimysql.execute(string, {}, function(result) {
              returnInfo = JSON.stringify({
                ...
                Họ: result[0].lastname.replace(/[^\w\s]/gi, ''),
                Giớitính: result[0].sex,
...
onNet("server-inventory-give", async (player, itemid, slot, amount, generateInformation, itemdata, openedInv) => {
  ...
  let values = `('${playerinvname}','${itemid}','${information}','${slot}','${creationDate}','${amount}')`
  /*  if (amount > 1) {
  ...
  let string = `SELECT item_id, id , amount, slot, information FROM user_inventory2 WHERE name='${playerinvname}' AND item_id='${itemid}'`;
  exports.ghmattimysql.execute(string, {}, function(result) {
    var itemids = ''
    var currammount = 0
	var currslot = 0
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      currammount = result[i].amount
	  currslot = result[i].slot
	  infor = result[i].information
    }
    if (itemids == itemid && infor == information) {
      let query = `UPDATE user_inventory2 SET amount = '${currammount}' + '${amount}' WHERE name = '${playerinvname}' AND item_id='${itemid}';`
...
onNet("server-inventory-refresh", async (player, sauce) => {
  ...
  let string = `SELECT  amount, id, name, item_id, information, slot, dropped, creationDate FROM user_inventory2 where name= '${player}' group by slot`;
  exports.ghmattimysql.execute(string, {}, function(inventory) {
    if (!inventory) {} else {
...
onNet("server-inventory-open", async (coords, player, secondInventory, targetName, itemToDropArray, sauce, TargetWeight) => {
  ...
  exports.ghmattimysql.execute(string, {}, function(inventory) {
    ...
    if (secondInventory == "1") {
      ...
      let string = `SELECT  amount, id, name, item_id, information, slot, dropped, creationDate FROM user_inventory2 WHERE name = '${targetinvname}' group by slot`;
...
onNet("server-inventory-close", async (player, targetInventoryName) => {
  ...
  if (targetInventoryName.indexOf("Drop") > -1 && DroppedInventories[targetInventoryName]) {
    if (DroppedInventories[targetInventoryName].used === false) {
      ...
      /* print('and5') */
      let string = `SELECT  amount, item_id, name, information, slot, dropped FROM user_inventory2 WHERE name='${targetInventoryName}' group by item_id `;
      exports.ghmattimysql.execute(string, {}, function(result) {
        /* var ammount = "0"
        for (let i = 0; i < result.length; i++) {
        ammount = result[i].amount
        }
        
        if (ammount == "0")
        {
        	 db(`DELETE FROM user_inventory2 WHERE name='${targetInventoryName}'`); 
        } */
...
onNet("server-inventory-move", async (player, data, coords) => {
  ...
  if (purchase) {
    ...
    info = await GenerateInformation(player, itemidsent)
    exports.ghmattimysql.execute(`SELECT item_id, id , name ,amount FROM user_inventory2 WHERE name='${targetName}' AND item_id='${itemidsent}'`, {}, function(result) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        invenname = result[i].name
        curr = result[i].amount
      }
      /* print('lavu')
      print(isWeapon) */
      if (invenname == null || isWeapon == true) {
        db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate,amount) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}','${amount}' );`);

      } else {
        db(`UPDATE user_inventory2 SET amount=amount + '${amount}' WHERE name='${targetName}' AND dropped='0' AND item_id='${itemidsent}'`);
      }
      /* print('crafting1') */
    });
  } else {
    ...
    if (targetName.indexOf("Drop") > -1 || targetName.indexOf("hidden") > -1) {
      exports.ghmattimysql.execute(`SELECT item_id, id , name ,amount FROM user_inventory2 WHERE name='${startname}' AND item_id='${itemidsent}'`, {}, function(result1) {

        var invenname = "0"
        var currammount = "0"
        for (let i = 0; i < result1.length; i++) {
          invenname = result1[i].name
          currammount = result1[i].amount
        }
        ...
        /* print('isweapons')
        print(isWeapon) */
        if (currammount == amount && isWeapon == false && itemids != null || currammount == "1" && itemids != null && isWeapon == false) {
          /* print('ang4') */
          db(`UPDATE user_inventory2 SET amount=amount + '${currammount}' WHERE name='${targetName}' AND item_id='${itemidsent}' `);
          db(`UPDATE user_inventory2 SET amount=amount - '${currammount}' WHERE name='${startname}' AND item_id='${itemidsent}'`);
        } 
		else 
		{
          /* print('ang3') */
      ...
      exports.ghmattimysql.execute(`SELECT item_id, id , name ,amount FROM user_inventory2 WHERE name='${startname}' AND item_id='${itemidsent}'`, {}, function(result1) {
        ...
        print(amount)
        print(crafting) */
        if (currammount == "1" && itemids != null && isWeapon == false || currammount == amount && isWeapon == false) {
          /* print('ang2') */
          /* db(`UPDATE user_inventory2 SET amount=amount + '${currammount}' WHERE name='${targetName}' AND item_id='${itemidsent}'`); */
          /* db(`UPDATE user_inventory2 SET amount=amount - '${currammount}' WHERE name='${startname}' AND item_id='${itemidsent}'`);  -- cách fix lấy đồ số lượng lớn hơn vào túi bị mất*/
		  if (startslot != targetslot && targetName == startname) {
			   /* print('ang2.1') */
			db(`UPDATE user_inventory2 SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${startslot}' and name='${startname}'`);  
		  }	else {
			   /* print('ang2.2') */
			   if (invenname == null) {
				   /* print('eu khong co thi moi add them') */
				    db(`UPDATE user_inventory2 SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${startslot}' and name='${startname}'`);
				   
			   }else{
				   /* print('co thi chi tru di') */
			...
			if (invenname == null){
			  ...
			  /* print('non321') */ // Fix việc bị âm đồ
				db(`UPDATE user_inventory2 SET amount=amount + '${currammount}' WHERE name='${targetName}' AND item_id='${itemidsent}'`);
				db(`UPDATE user_inventory2 SET amount=amount - '${currammount}' WHERE name='${startname}' AND item_id='${itemidsent}'`);
			}
        }
...
onNet("server-inventory-stack", async (player, data, coords) => {
  ...
  if (purchase) {
    ...
    if (isWeapon) {
		...
		if(invenname != null){
			/* db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate, amount) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' ),'${amount}';`); */
			db(`UPDATE user_inventory2 SET amount=amount WHERE name='${targetName}' AND dropped='0' AND item_id='${itemidsent}'`);	
		}	else	{
			db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate, amount) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' ),'${amount}';`);
		}});
    ...
    if (!PlayerStore) {
		...
		{
			db(`UPDATE user_inventory2 SET amount=amount + '${moveAmount}' WHERE name='${targetName}' AND dropped='0' AND item_id='${itemidsent}'`);
		}
    ...
    if (targetName.indexOf("Drop") > -1 || targetName.indexOf("hidden") > -1) {
      exports.ghmattimysql.execute(`SELECT item_id, id , name ,amount, creationDate FROM user_inventory2 WHERE name='${targetName}' AND item_id='${itemidsent}'`, {}, function(result) {
        var itemids = "0"
        var invenname = null
        var curr = "0"
		var infor = "0"
       ...
       /*  print('this4') */
        ...
        if (invenname == null) {
          /* print('have') */
		  if(isWeapon){
			  /* db(`INSERT INTO user_inventory2 (SELECT item_id,id,name,information,slot,dropped,creationDate,quality,amount FROM user_inventory2 WHERE name='${originInventory}' AND dropped='0' AND item_id='${itemidsent}' AND slot='${originSlot}');`); */
			  let string = `SELECT information, creationDate FROM user_inventory2 WHERE name='${originInventory}' AND dropped='0' AND item_id='${itemidsent}' AND slot='${originSlot}'`;
			  exports.ghmattimysql.execute(string, {}, function(result) {
				...
				for (let i = 0; i < result.length; i++) {
				  infor = result[i].information
				  date = result[i].creationDate
				}
			  /* db(`INSERT INTO user_inventory2 (item_id, name, information, slot,creationDate, quality,amount) SELECT item_id,name,information,slot,creationDate,quality,amount FROM user_inventory2 WHERE name='${originInventory}' AND dropped='0' AND item_id='${itemidsent}' AND slot='${originSlot}';`); */
			db(`UPDATE user_inventory2 SET amount=amount - '${moveAmount}' WHERE name='${originInventory}' AND dropped='0' AND item_id='${itemidsent}' AND slot='${originSlot}'`);
			db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate, amount, dropped) VALUES ('${itemidsent}','${targetName}','${infor}','${targetslot}','${date}' ,'${moveAmount}', '1');`);
          ...
          if (originInventory == targetName && moveAmount > 0 && targetslot != originSlot) {
			...
			/* print('drop1.2') */
            db(`UPDATE user_inventory2 SET amount=amount - '${moveAmount}' WHERE name='${originInventory}' AND dropped='0' AND item_id='${itemidsent}'`);
            db(`UPDATE user_inventory2 SET amount=amount + '${moveAmount}' WHERE name='${targetName}' AND dropped='1' AND item_id='${itemidsent}'`);
			}
          }
        }
      });
    } else {
      let string = `SELECT item_id, id , name ,amount FROM user_inventory2 WHERE name='${targetName}' AND item_id='${itemidsent}'`;
      exports.ghmattimysql.execute(string, {}, function(result) {
        ...
        if (invenname == null) {
			 /* print('chưa tồn tại nên add thêm') */
          /* db(`UPDATE user_inventory2 SET amount = amount + '${moveAmount}' WHERE name='${targetName}' AND dropped='0' AND slot='${targetslot}' `); */
		  /* fix lỗi cất số lượng qua cốp thì mất bị hư dropped từ 1 thành 0  */
		  /* let string = `SELECT information, creationDate FROM user_inventory2 WHERE name='${originInventory}' AND dropped='0' AND item_id='${itemidsent}' AND slot='${originSlot}'`; */
		  let string = `SELECT information, creationDate, amount FROM user_inventory2 WHERE name='${originInventory}' AND item_id='${itemidsent}' AND slot='${originSlot}'`;
			  exports.ghmattimysql.execute(string, {}, function(result) {
				...
				for (let i = 0; i < result.length; i++) {
				  infor = result[i].information
				  date = result[i].creationDate
				  currentamount = result[i].amount
				}
				// Fix lỗi đúp đồ cốp (hoạt động tốt)
				if (moveAmount <= currentamount)
				{
				db(`UPDATE user_inventory2 SET amount=amount - '${moveAmount}' WHERE name='${originInventory}' AND item_id='${itemidsent}'`);
			/* db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate, amount, dropped) VALUES ('${itemidsent}','${targetName}','${infor}','${targetslot}','${date}' ,'${moveAmount}', '0');`); */
				db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate, amount, dropped) VALUES ('${itemidsent}','${targetName}','${infor}','${targetslot}','${creationDate}' ,'${moveAmount}', '0');`);
				}
			});	
         
          /* db(`UPDATE user_inventory2 SET amount=amount - '${moveAmount}' WHERE name='${originInventory}' AND item_id='${itemidsent}'`);
          db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate, amount, dropped) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' ,'${moveAmount}', '0');`); */
          ...
          if (originInventory == targetName && moveAmount > 0 && targetslot != originSlot) {
            ...
            /* print('add to cvop') */
            db(`UPDATE user_inventory2 SET amount=amount - '${moveAmount}' WHERE name='${originInventory}' AND item_id='${itemidsent}'`);
            db(`UPDATE user_inventory2 SET amount=amount + '${moveAmount}' WHERE name='${targetName}' AND item_id='${itemidsent}'`);

          }
          /* db(`UPDATE user_inventory2 SET amount = amount + '${moveAmount}' WHERE name='${targetName}' AND dropped='0' AND item_id='${itemidsent}'`); 
          db(`UPDATE user_inventory2 SET amount = '${amountRemaining}' WHERE name='${originInventory}' AND dropped ='1' AND slot='${originSlot}' AND item_id='${itemidsent}'`);
           */
        }
      });
    }
