...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon)
    ...
    if (itemid == "vodka" or itemid == "beer" or itemid == "whiskey") then
        AttachPropAndPlayAnimation("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,6000,"Uống","changethirst",true,itemid,playerVeh)
    ...
    if (itemid == "heavyammo") then
        local finished = exports["np-taskbar"]:taskBar(5000,"Đang nạp",true,true,playerVeh)
        if (finished == 100) then
	...
	if (itemid == "huntingammo") then
        local finished = exports["np-taskbar"]:taskBar(10000,"Reloading",true,true,playerVeh)
        if (finished == 100) then
            TriggerEvent("actionbar:ammo",1285032059,5,true)
            remove = true
        end
    ...
    if (itemid == "taserammo") then
        local finished = exports["np-taskbar"]:taskBar(2000,"Đang nạp",true,true,playerVeh)
        if (finished == 100) then
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        local cuntfuck = #(GetEntityCoords(PlayerPedId()) - vector3(39.446071, -1749.563, 29.30294))
		if cuntfuck < 2.5 then
