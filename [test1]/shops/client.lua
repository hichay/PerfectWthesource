...
local EMT_Shops = {
	{ ['x'] = 306.49, ['y'] = -601.31, ['z'] = 43.29, ['h'] = 53.413696  },
	{ ['x'] = 1828.3546, ['y'] = 3672.8305, ['z'] = 34.271053, ['h'] = 53.413696  },
}
...
local hunting_shops = {
	{ ['x'] = -675.7781, ['y'] = 5836.1059, ['z'] = 17.331432, ['h'] = 53.413696  },
}
...
local twentyfourseven_shops = {
	...
	{ ['x'] = 439.36697, ['y'] = -979.0026, ['z'] = 30.689565 },
    ...
    { ['x'] = 191.06288, ['y'] = -890.5704, ['z'] = 30.713088 }, -- legion square
	{ ['x'] = 1166.0147, ['y'] = 2707.7802, ['z'] = 38.157699 },
	{ ['x'] = -2969.487, ['y'] = 390.78109, ['z'] = 15.043312 },
	{ ['x'] = -3247.092, ['y'] = 1001.0137, ['z'] = 12.830707 },
	{ ['x'] = -2550.102, ['y'] = 2317.163, ['z'] = 33.21566 },
	{ ['x'] = -1111.374, ['y'] = 2694.3085, ['z'] = 18.554128 },
	{ ['x'] = -2195.55, ['y'] = 4289.7783, ['z'] = 49.173503 },
	{ ['x'] = 314.9768, ['y'] = -587.9898, ['z'] = 43.284065 },
	{ ['x'] = -679.541, ['y'] = 5837.4458, ['z'] = 17.340139 }, --hunting store
	{ ['x'] = 165.0545, ['y'] = 6642.217, ['z'] = 30.69 }, --hunting store
...
function setShopBlip()
	...
	-- for station,pos in pairs(weashop_locations) do
		...
		-- BeginTextCommandSetBlipName("STRING")
		-- AddTextComponentString('Silah Dükkanı')
		-- EndTextCommandSetBlipName(blip)
		-- SetBlipAsShortRange(blip,true)
	...
	for k,v in ipairs(twentyfourseven_shops)do
		...
		AddTextComponentString("Cửa hàng tạp hóa")
		EndTextCommandSetBlipName(blip)
	end
	...
	for k,v in ipairs(hunting_shops)do
		local blip = AddBlipForCoord(v.x, v.y, v.z)
		...
		SetBlipAsShortRange(blip, true)
		BeginTextCommandSetBlipName("CUSTOM_TEXT")
		AddTextComponentString("Cửa hàng thợ săn")
		EndTextCommandSetBlipName(blip)
	end		
...
function DisplayHelpText(msg)
	...
	if string.match(msg, "~INPUT_CONTENT~") then
        msg = msg:gsub("~INPUT_CONTENT~", "[E]")
    end
    if string.match(msg, "~INPUT_CONTEXT~") then
        msg = msg:gsub("~INPUT_CONTEXT~", "[E]")
    end
    if string.match(msg, "~INPUT_DETONATE~") then
        msg = msg:gsub("~INPUT_DETONATE~", "[G]")
    end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		for i,b in ipairs(weashop_blips) do
			...
			if dstscan < 1.0 then
				...
				if IsPedInAnyVehicle(PlayerPedId(), true) == false and dstscan < 0.25 then
					DisplayHelpText("Marketi açmak için ~INPUT_CONTEXT~ tuşuna bas.")
		...
		-- 	if(Vdist( , -115.18, 74.19, pos.x, pos.y, pos.z) < 2.0)then
		...
		for k,v in ipairs(EMT_Shops) do
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 10.0)then
				...
				if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
					...
					DisplayHelpText("Nhấn [E] để mở ~g~cửa hàng.")
					if IsControlJustPressed(1, 38) then	
		...
		if(Vdist(835.16, -1295.19, 24.32, pos.x, pos.y, pos.z) < 20.0)then
			found = true
			DrawMarker(27, 835.16, -1295.19, 24.32 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
			if(Vdist(835.16, -1295.19, 24.32, pos.x, pos.y, pos.z) < 2.0)then
				local job = exports["isPed"]:isPed("myjob")
				DisplayHelpText("Press ~INPUT_CONTEXT~ to open the ~g~shop.")
				if IsControlJustPressed(1, 38) and job == "police" then	
					TriggerEvent("server-inventory-open", "10", "Shop");	
    	...
    	if(Vdist(1775.6893310547,2593.6455078125,45.723571777344, pos.x, pos.y, pos.z) < 2.0)then
			...
			if(Vdist(1775.6893310547,2593.6455078125,45.723571777344, pos.x, pos.y, pos.z) < 2.0)then
				...
				if IsControlJustPressed(1, 38) and (Vdist(1775.6893310547,2593.6455078125,45.723571777344, pos.x, pos.y, pos.z) < 2.0) then
					local finished = exports["np-taskbar"]:taskBar(10000,"Making a god slushy...")
      				if (finished == 100) then
		...
		if(Vdist(-632.64, 235.25, 81.89, pos.x, pos.y, pos.z) < 2.0)then
			found = true
		...
		for k,v in ipairs(tool_shops) do
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
				...
				DrawMarker(27, v.x, v.y, v.z - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
				if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 3.0)then
					DisplayHelpText("Nhấn ~INPUT_CONTEXT~ để mở cửa hàng")
					if IsControlJustPressed(1, 38) then
		...
		for k,v in ipairs(hunting_shops) do
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
				...
				DrawMarker(27, v.x, v.y, v.z - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
				if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 3.0)then
					DisplayHelpText("Nhấn [E] để mở cửa hàng")
					if IsControlJustPressed(1, 38) then
						TriggerEvent("server-inventory-open", "12", "Shop");
						Wait(1000)
				    end
