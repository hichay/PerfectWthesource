...
on('esx:playerLoaded', (broughtData) => {
	let cid = PlayerPedId()
...
on("__cfx_nui:GiveItem", (data, cb) => {
	...
	let itemdata = data[5]
	let returnData = data[6]

	emit("hud-display-item",id,"Nhận được",amount)
	print(itemdata)
...
function GiveItem(itemid, amount, generateInformation, nonStacking, itemdata, returnData = '{}') {
    ...
    if (slot != 0) {
        ...
        emitNet('server-inventory-give',plySteam,itemid,slot,amount,generateInformation,Object.assign({}, itemdata),openedInv,returnData,
        );
