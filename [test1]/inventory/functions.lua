...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon)
    ...
    if justUsed then
	    ...
	    ESX.ShowNotification("Tried using " .. retardCounter .. " items in < 500ms ")
        end
        return
    end
    ...
    if (not hasEnoughOfItem(itemid,1,false)) then
		...
		ESX.ShowNotification("You dont appear to have this item on you?")
        justUsed = false
        retardCounter = 0
        lastCounter = 0
        return
...
exports('hasEnoughOfItem', function(itemid,amount,shouldReturnText,checkQuality)
...
function getJobsss()
    return ESX.PlayerData.job ~= nil and ESX.PlayerData.job
end
