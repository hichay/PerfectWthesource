...
RegisterCommand('+inventoryOpen2', function()
	TriggerEvent("np-business:examineItem")
end, false)

RegisterCommand('+inventoryOpen3', function()
	TriggerEvent("np-business:repairItem")
end, false)
...
RegisterNetEvent('cash:client:remove')
AddEventHandler('cash:client:remove', function(amount321)
    TriggerServerEvent('cash:removehello',amount321)
end)

local LastUsedItem = 0
local LastUsedItemId = "ciggy"
local plySteam = false

RegisterNetEvent('inventory-client-identifier')
AddEventHandler('inventory-client-identifier', function(steam)
    plySteam = steam
end)
...
AddEventHandler('inventory:DegenLastUsedItem', function(percent)
    ...
    TriggerServerEvent("inventory:degItem",LastUsedItem,percent,LastUsedItemId,plySteam)
end)
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if justUsed then
		...
		TriggerEvent("ESX:Notify","Đang cố spam sử dụng đồ "..retardCounter.." trong thời gian <500ms")
        end
        return
    end
    ...
    if (not hasEnoughOfItem(itemid,1,false)) then
		...
		TriggerEvent("ESX:Notify","Không có món đồ này","error")
        justUsed = false
        retardCounter = 0
        lastCounter = 0
        return
    ...
    if (itemid == "tuner") then
      ...
      local finished = exports["pw-taskbar"]:taskBar(2000,"Connecting Tuner Laptop",true,true,playerVeh)
    ...
    if itemid == "motelkey"  then
		TriggerEvent('pw-motelsbusiness:UseKey')
    end
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["pw-taskbar"]:taskBar(3000,"Placing LSD Strip on 👅",true,true,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "matrixredpill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["pw-taskbar"]:taskBar(3000,"Taking the Red Pill",false,false,playerVeh)
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["pw-taskbar"]:taskBar(25000,"Decrypting Data",true,true,playerVeh)
          if (finished == 100) then
    ...
    if (itemid == "weedoz") then
      ...
      local finished = exports["pw-taskbar"]:taskBar(5000,"Packing Q Bags",true,true,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation","lighter")
          local finished = exports["pw-taskbar"]:taskBar(2000,"Starting Fire",true,true,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "joint" or itemid == "joint2") then
        local finished = exports["pw-taskbar"]:taskBar(2000,"Hút cuốn",true,true,playerVeh)
        if (finished == 100) then
            Wait(200)
	    local weedStrength = 1.0
            TriggerEvent("animation:PlayAnimation","weed")
	        TriggerEvent("fx:run", "weed", 150,  weedStrength)	
            TriggerEvent("Evidence:StateSet",3,600)
            TriggerEvent("Evidence:StateSet",4,600)        
            TriggerEvent("stress:timed",200000,"WORLD_HUMAN_SMOKING_POT")
            remove = true
        end
    end
     ...
     if (itemid == "Gruppe6Card") then
        ...
        if targetVehicle ~= 0 and GetHashKey("stockade") == GetEntityModel(targetVehicle) then
            ...
            if aDist < 2.0 then
                ...
                local finished = exports["pw-taskbar"]:taskBar(timeToOpen,"Unlocking Vehicle",true,true,playerVeh)
                if finished == 100 then
                    remove = true
    ...
    if (itemid == "usbdevice") then
        local finished = exports["pw-taskbar"]:taskBar(15000,"Scanning For Networks",true,true,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "heavyammo") then
        local finished = exports["pw-taskbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "pistolammo") then
        local finished = exports["pw-taskbar"]:taskBar(2500,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "flamethrowerammo") then
        local finished = exports["pw-taskbar"]:taskBar(5000,"Reloading flamethrower",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "huntingammo") then
        local finished = exports["pw-taskbar"]:taskBar(10000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "rpgammo") then
        local finished = exports["pw-taskbar"]:taskBar(25000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "taserammo") then
        local finished = exports["pw-taskbar"]:taskBar(2000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "armor") then
        local finished = exports["pw-taskbar"]:taskBar(10000,"Armor",true,true,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "cbrownie" or itemid == "cgummies") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["pw-taskbar"]:taskBar(3000,"Consuming edibles 😉",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "bodybag") then
        local finished = exports["pw-taskbar"]:taskBar(10000,"Opening bag",true,true,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
            local finished = exports["pw-taskbar"]:taskBar(10000,"Opening trash bag",true,true,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = exports["pw-taskbar"]:taskBar(20000,"Opening the crate (ESC to Cancel)",true,true,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "organcooler") then
        local finished = exports["pw-taskbar"]:taskBar(5000,"Opening cooler",true,true,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem",1,false)) then
            local finished = exports["pw-taskbar"]:taskBar(10000,"Opening bank box.",true,true,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey",1,false)) then
            local finished = exports["pw-taskbar"]:taskBar(5000,"Opening briefcase.",true,true,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            local finished = exports["pw-taskbar"]:taskBar(15000,"Opening supply crate.",true,true,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "xmasgiftcoal") then
        local finished = exports["pw-taskbar"]:taskBar(15000, "Opening Gift", true, true, playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "fishingtacklebox") then
        local finished = exports["pw-taskbar"]:taskBar(5000,"Opening",true,true,playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
            ...
            CreateThread(function()
                while finished ~= 100 and not cancelNos do
                    ...
                    if GetEntitySpeed(GetVehiclePedIsIn(player, false)) > 11 then
                        exports["pw-taskbar"]:closeGuiFail()
                        cancelNos = true
                    end
                end
            end)
    ...
    if (itemid == "lockpick") then
        -- local myJob = exports["isPed"]:isPed("myJob")
        -- if myJob ~= "news" then
            -- TriggerEvent("inv:lockPick",true,inventoryName,slot)
    ...
    if (itemid == "securityblue" or itemid == "securityblack" or itemid == "securitygreen" or itemid == "securitygold" or itemid == "securityred")  then
		--print('card')
        TriggerEvent("robbery:scanLock",true,itemid)       
    end
    ...
    if (itemid == "ciggy") then
		if hasEnoughOfItem("lighter",1,false) then
			local finished = exports["pw-taskbar"]:taskBar(1000,"Đốt lửa",false,false,playerVeh)
			if (finished == 100) then
				Wait(300)
				-- TriggerEvent("animation:PlayAnimation","smoke")
				-- TriggerEvent("stress:timed",50000,"WORLD_HUMAN_SMOKING")
				TriggerEvent('pw-smoke:CigarettesLightingAnim')
				remove = true
			end
		else
			TriggerEvent("ESX:Notify","Không có bật lửa","error")
		end
    end
	
	if (itemid == "redpack") then
		local finished = exports["pw-taskbar"]:taskBar(1000,"Mở bao thuốc",false,false,playerVeh)
		if (finished == 100) then
			TriggerEvent('pw-smoke:CigarettesPackAnim')
			Wait(3000)
			TriggerEvent("player:receiveItem",'ciggy',10)
			remove = true
		end
	end
    ...
    if (itemid == "cigar") then
        local finished = exports["pw-taskbar"]:taskBar(1000,"Đốt lửa",true,true,playerVeh)
        if (finished == 100) then
            Wait(300)
    ...
    if (itemid == "cokeline") then
        ...
        if itemInfo and itemInfo.quality > 0 then
            ...
            if currentUses == 0 then
                ...
                local finished = exports["pw-taskbarskill"]:taskBarSkill(3000, math.random(15, 20))
                if (finished == 100 and hasEnoughOfItem(itemid, 1, false)) then
    ...
    if (itemid == "crackpipe") then
        ...
        if itemInfo and itemInfo.quality > 0 then
            ...
            if currentUses == 0 then
                ...
                local finished = exports["pw-taskbarskill"]:taskBarSkill(2400, math.random(12, 18))
                if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                    currentUses = currentUses - 1
                    TriggerEvent("inventory:updateItem", itemid, slot, json.encode({ uses = currentUses }))
	...
	TriggerEvent("fx:run", "crack", 180, -1, false)
    end
	...
	if (itemid == "morphine") then
        --TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49,2000,"Applying IFAK","healed:useOxy",true,itemid,playerVeh)
		TriggerEvent("pw-hospital:items:morphine")
		remove = true
    end
	
	if (itemid == "aspirin") then
        --TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49,2000,"Applying IFAK","healed:useOxy",true,itemid,playerVeh)
		TriggerEvent("pw-hospital:items:aspirin")
		remove = true
    end
    ...
    or itemid == "experiment_09") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["pw-taskbar"]:taskBar(3000,"Downing content of bottle. 💊",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "pillbox") then
        local finished = exports["pw-taskbar"]:taskBar(5000, "Opening pill box", false, false, playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
	...
	if (itemid == "oxynew") then
        local finished = exports["pw-taskbar"]:taskBar(5000,"Uống thuốc",true,true,playerVeh)
        if (finished == 100) then
			TriggerEvent("animation:PlayAnimation","pill")
            Wait(300)
    ...
    if (itemid == "sandwich" or itemid == "hamburger") then
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Đang ăn","changehunger",true,itemid,playerVeh)
		TriggerEvent('esx_status:add','hunger', 400000)
    end
	
	if (itemid == "thitthu1" or itemid == "thitbonuong") then
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Đang ăn","changehunger",true,itemid,playerVeh)
		TriggerEvent('esx_status:add','hunger', 600000)
		TriggerEvent('esx_status:remove','stress', 200000)
    end
	
	if itemid == "thitthu2" then
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Đang ăn","changehunger",true,itemid,playerVeh)
		TriggerEvent('esx_status:add','hunger', 600000)
		TriggerEvent('esx_status:remove','stress', 400000)
    end
	
	if itemid == "thitthu3" then
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Đang ăn","changehunger",true,itemid,playerVeh)
		TriggerEvent('esx_status:add','hunger', 800000)
		TriggerEvent('esx_status:remove','stress', 600000)
    end

    if (itemid == "cola" or itemid == "water") then
        AttachPropAndPlayAnimation("amb@world_human_drinking@beer@female@idle_a", "idle_e", 49,6000,"Đang uống","changethirst",true,itemid,playerVeh)
    ...
    if (itemid == "jailfood" or itemid == "bleederburger" or itemid == "heartstopper" or itemid == "torpedo" or itemid == "meatfree" or itemid == "moneyshot" or itemid == "fries") then
        AttachPropAndPlayAnimation("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Đang ăn","inv:wellfed",true,itemid,playerVeh)
    ...
    if itemid == "ketamine" then
        local finished = exports["pw-taskbarskill"]:taskBarSkill(2000, math.random(10, 15))
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("attachItemObjectnoanim","drugpackage01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "methbag") then
        local finished = exports["pw-taskbarskill"]:taskBar(2500,10)
        if (finished == 100) then  
            TriggerEvent("attachItemObjectnoanim","crackpipe01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["pw-taskbarskill"]:taskBarSkill(1000, math.random(7, 12))
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("pw-police:uncuff")
        end
        lockpicking = false
        remove = true
    end
    ...
    if itemid == "tattooremover" then
        ...
        if IsEntityAPed(target) and IsPedAPlayer(target) then
            ...
            local finished = exports["pw-taskbar"]:taskBar(60000, "Removing tattoos")
	 ...
	 if itemid == "buddhaletter" then
        if GetEntityModel(PlayerPedId()) == `ig_buddha` or GetEntityModel(PlayerPedId()) == `g_m_m_buddha` then
            local finished = exports["pw-taskbar"]:taskBar(5000,"Opening letter.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
        ...
        if hasEnoughOfItem("buddhablade",1,false) and hasEnoughOfItem("buddhaguard",1,false) and hasEnoughOfItem("buddhahilt",1,false) then
            if hasEnoughOfItem("buddhamedalion",1,false) then
                ...
                if #(playerPos - vector3(-172.54, 319.13, 87.35)) < 10 then
                    local finished = exports["pw-taskbar"]:taskBar(30000,"Putting sword together.",false,false,playerVeh)
                    if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if itemid == "buddhashovel" then
        ...
        if #(playerPos - vector3(-1911.78, 1388.1, 219.35)) < 10 then
            if BuddhaIsItemFound1 == false then
                ...
                local finished = exports["pw-taskbar"]:taskBar(15000, "Digging for something.", false, true, false, false, nil, 5.0, PlayerPedId())
                ClearPedTasks(PlayerPedId())
                if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    -- if (itemid == "firstimegift" or itemid == "onlinelv1" or itemid == "onlinelv2" or itemid == "onlinelv3" or itemid == "loginreward1" or itemid == "loginreward2") then
		-- TriggerServerEvent('moon_gachapon:Continue',itemid)
		-- remove = true
        
    -- end 
	
	if (itemid == "snorkelling_gear") then
		TriggerClientEvent('azael_ui-diving:snorkellingDiving', source)       
    end 
	
	if (itemid == "scuba_oxygen_tank") then
		TriggerClientEvent('esx_status:add',source, 'scubaOxygen',500000)
		remove = true
        
    end

	-- if (itemid == "scuba_gear") then
		-- TriggerClientEvent('azael_ui-diving:scubaDiving', source)
...
function getInformationFromFirstItem(itemToFind)
  local foundItem = exports["pw-inventory"]:GetInfoForFirstItemOfName(itemToFind)
  return foundItem or nil
end
...
function GetInfoForFirstItemOfName(name)
    for i,v in pairs(clientInventory) do
        if (tostring(v.item_id) == tostring(name)) then
            local info = v.quality
            return info
        end
    end
    return "No information stored";
...
function GetItemInfoName(name)
    for i,v in pairs(clientInventory) do
        if (tostring(v.item_id) == tostring(name)) then
            local info = v.information
            return info
        end
    end
    return "No information stored";
...
AddEventHandler('np-business:repairItem', function()
    ...
    local itemListInfo = json.decode(exports["pw-inventory"]:itemListInfo(info.item_id))
    local allowed = true
    ...
    local failed = ( (info.amount > 1 and "You can only repair 1 item at a time" ) or (not QualityCheck and "Can only repair items between 0-80%. ") or (not itemListInfo and "Item is unrepairable. ") or (not allowed and "Not enough materials. "))
	...
	TriggerServerEvent("inventory:repairItem",info.id,80,info.item_id,plySteam)
...
AddEventHandler('np-business:examineItem', function()
	...
	local failed = ( (info.amount > 1 and "You can only repair 1 item at a time" ) or (not QualityCheck and "Can only repair items between 0-80%. ") or (not itemListInfo and "Item is unrepairable. ") )
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh,itemreturn,itemreturnid)
    ...
    if timer > 0 then
        local finished = exports["pw-taskbar"]:taskBar(timer,message,true,true,playerVeh)
...
function getQuantity(itemid, checkQuality, metaInformation)
  local amount = 0
  for i,v in pairs(clientInventory) do
	--TriggerEvent("table",clientInventory)
...
-- exports('hasEnoughOfItem', function(itemid,amount,shouldReturnText)
...
AddEventHandler('inventory:DegenItemType', function(percent,itemid)
	...
	--print(itemFound[1].id)
    TriggerServerEvent("inventory:degItem",itemFound[1].id,percent,itemid,plySteam)
end)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            while dist > 2.0 and count > 0 do
                ...
                Wait(1)
                count = count - 1
                DrawText3Ds(leftfront["x"],leftfront["y"],leftfront["z"],"Đứng ở đây đê lockpick.")
            end
            ...
            -- local finished = exports["pw-taskbarskill"]:taskBar(25000,3)
            ...
            --local finished = exports["pw-taskbarskill"]:taskBar(2200,10)
			local finished = exports["pw-lockpicking"]:lockpick(1,15,math.random(10,15),5)
            if finished == 0 then
                 lockpicking = false
            ...
            if finished == 100 then
                ...
                if #(GetEntityCoords(targetVehicle) - GetEntityCoords(PlayerPedId())) < 10.0 and targetVehicle ~= 0 and GetEntitySpeed(targetVehicle) < 5.0 then
                    ...
                    TriggerEvent("ESX:Notify", "Cửa xe đã được phá thành công.","info")
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            -- local finished = exports["pw-taskbarskill"]:taskBar(math.random(5000,25000),math.random(10,20))
            -- if finished ~= 100 then
                 -- lockpicking = false
                -- return
            -- end
            ...
            --local finished = exports["pw-taskbarskill"]:taskBar(1500,math.random(1,3))
			local finished = exports["pw-lockpicking"]:lockpick(1,15,math.random(5,10),5)
            if finished == 0 then
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["pw-taskbarskill"]:taskBar(15000,math.random(10,20))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
                ClearPedTasks(playerped)
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["pw-taskbarskill"]:taskBar(timer,math.random(5,15))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
...
function getJobsss()
    return ESX.PlayerData.job.name ~= nil and ESX.PlayerData.job.name
end

AddEventHandler("pw-inventory:itemUsed", function(name, info)
