...
on('inventory-open-request', async () => {
	...
	if (isInVehicle) {
        ...
        let distanceRear = GetDistanceBetweenCoords(startPosition[0],startPosition[1],startPosition[2],trunkCoords[0],trunkCoords[1],trunkCoords[2],);
...
on('server-inventory-open', (target, name, targetWeight, targetSlots) => {
    ...
    emitNet('server-inventory-open', GetEntityCoords(PlayerPedId()), plySteam, target, name, null, null, targetWeight, targetSlots);
});
...
exports('setPlayerWeight', (plySteam, weight) => {
    maxPlayerWeight = weight;
    emitNet('np-inventory:server:weightChange', plySteam, weight);
    SendNuiMessage(JSON.stringify({ response: 'playerWeight', personalMaxWeight: weight }));
});
