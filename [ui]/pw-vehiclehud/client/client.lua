	...
	while true do
		...
		if IsPauseMenuActive() or IsScreenFadedOut() then
			...
			if not IsPedInAnyVehicle(ped) then
				sleep = 1000
				SendNUIMessage({ hud = showHud, car = false, hour = hour, minute = minute, street = street, health = tonumber(health), armour = tonumber(armour), resourcename = resourcename })
			else
				sleep = 200
				showRadar = true
				...
				SendNUIMessage({ hud = showHud, car = true, hour = hour, minute = minute, street = street, health = tonumber(health), armour = tonumber(armour), fuel = tonumber(fuel), speed = tonumber(speed), seatbelt = seatbelt, gear = gear, carheath = engine, resourcename = resourcename })
			end
		end
		Citizen.Wait(sleep)
	end
