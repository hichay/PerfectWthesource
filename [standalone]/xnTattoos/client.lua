...
RegisterCommand("tattiemxam", function(source, args, rawCommand)
	JayMenu.CloseMenu()
end, false)

RegisterCommand("loadhinhxam", function(source, args, rawCommand)
	TriggerEvent("skinchanger:modelLoaded")
end, false) 
...
function CreateScale(sType)
	...
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Góc nhìn", 44, "Sáng/Mo", {90, 89}, "Mua/Tháo hình", 191)
		scaleString = sType
	end
end
...
Citizen.CreateThread(function()
    ...
    JayMenu.SetSubTitle('tattoo', "Danh mục")
			...
			for _,interiorId in ipairs(Config.interiorIds) do
				if GetInteriorFromEntity(PlayerPedId()) == interiorId then
					...
					if not IsPedInAnyVehicle(PlayerPedId(), false) then
						ESX.ShowHelpNotification("Nhấn [E] để mở shop")
...
function ButtonMessage(text)
    BeginTextCommandScaleformString("CUSTOM_TEXT1")
