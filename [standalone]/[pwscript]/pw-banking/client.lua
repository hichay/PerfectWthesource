    ...
    while true do
        ...
        if NearBank() and not isBankOpened and NearBank() ~= "update" then
        	if not Config.okokTextUI then
            	ESX.ShowHelpNotification("Nhấn ~INPUT_PICKUP~ để mở ~b~Ngân hàng")
            else
            	inRange = true
            end
...
RegisterNUICallback("action", function(data, cb)
	...
	elseif data.action == "deposit" then
		if tonumber(data.value) ~= nil then
			...
			TriggerEvent("ESX:Notify", "Ô số đang trống","error")
		end
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
					...
					TriggerEvent("ESX:Notify", "IBAN này đã có người sử dụng","error")
				end
			end, iban)
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				TriggerEvent("ESX:Notify", "IBAN chỉ có dạng số","error")
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				TriggerEvent("ESX:Notify", "Mã pin đã thay đổi thành công","info")
			else
				--exports['okokNotify']:Alert("BANK", "Your PIN needs to be 4 digits long", 5000, 'info')
				TriggerEvent("ESX:Notify", "Mã pin phải dài 4 số","info")
			end
		else
			--exports['okokNotify']:Alert("BANK", "You can only use numbers", 5000, 'info')
			TriggerEvent("ESX:Notify", "Mã pin chỉ chấp nhận dưới dạng số","info")
		end
	end
end)
