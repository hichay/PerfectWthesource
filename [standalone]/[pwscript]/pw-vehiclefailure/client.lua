...
local function notification(msg)
	SetNotificationTextEntry("CUSTOM_TEXT")
...
-- AddEventHandler('iens:repair', function()
	-- if isPedDrivingAVehicle() then
		...
		-- if IsNearMechanic() then
			...
			-- notification("~g~Xe cua ban da duoc sua voi chi phi $ 1000!")
			-- return
		-- end
		-- if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			-- if GetVehicleOilLevel(vehicle) > 0 then
				...
				-- notification("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", Tim tho may ngay!")
				-- fixMessagePos = fixMessagePos + 1
				-- if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			-- else
				-- notification("~r~Xe cua ban da hu hai nghiem trong. Khong the tu sua chua!")
			-- end
		-- else
		...
		-- end
	-- else
		-- notification("~y~Ban phai ngoi trong xe moi co the sua chua")
	-- end
-- end)

RegisterNetEvent('iens:notAllowed')
AddEventHandler('iens:notAllowed', function()
	notification("~y~Ban khong the sua duoc xe nay")
end)
