...
AddEventHandler("pw-inventory:itemUsed", function(itemId, itemInfo, inventoryName, slot)
    if itemId == "harness" then
        ...
        local class = GetVehicleClass(GetVehiclePedIsUsing(PlayerPedId()))
        infoquality = exports["pw-inventory"]:GetItemInfo(slot)
        if inveh and infoquality.quality > 0 then
            if not harnessOn then
                LocalPlayer.state:set("inv_busy", true, true)
                local finished = exports["pw-taskbar"]:taskBar(1, "Attaching Race Harness")
                ...
                TriggerEvent('hud:client:UpdateHarness', infoquality.quality)
            else
                LocalPlayer.state:set("inv_busy", true, true)
                local finished = exports["pw-taskbar"]:taskBar(10000, "Removing Race Harness")
                if finished == 100 then
                    LocalPlayer.state:set("inv_busy", false, true)
                    ToggleHarness()
                end
            ...
            TriggerEvent("DoLongHudText","Bạn không ngồi trong xe", 2)
        end
    end
end)
...
function ToggleHarness()
    if harnessOn then
        harnessOn = false
        TriggerEvent('hud:client:activeHarness', false)
    else
        harnessOn = true
        TriggerEvent('hud:client:activeHarness', true)
        ToggleSeatbelt()
    end
end
