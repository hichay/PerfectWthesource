Config = {} 
Config.DamageNeeded = 400.0 -- 100.0 being broken and 1000.0 being fixed a lower value than 100.0 will break it
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
					--if vehClass ~= 13 or vehClass ~= 14 or vehClass ~= 15 or vehClass ~= 16 then
						--ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Nhấn [~g~SHIFT~w~] và [~g~E~w~] để đẩy xe', 0.7)
						DrawText3Ds(Vehicle.Coords.x, Vehicle.Coords.y, Vehicle.Coords.z, 'Nhấn [~g~SHIFT~w~] và [~g~E~w~] để đẩy xe')
					--end
                end
