...
AddEventHandler('pw-doors:jobChanged', function(currentJob, previousJob)
...
function hasSecuredAccess(pId, pType)
    ...
    if      (secured.access.job and secured.access.job[CurrentJob] or false)
        or  (secured.access.job["police"] ~= nil and isPD(job))
    ...
    if secured.item ~= nil then
        accessCheckCacheTimer[pType][pId] = 0
        for i, v in pairs(secured.item) do
