...
RegisterNetEvent('pw-smoke:JointLightingAnim')
AddEventHandler('pw-smoke:JointLightingAnim', function(source)
	...
	AttachEntityToEntity(jointnolight, playerPed, GetPedBoneIndex(playerPed, 64097), 0.020, 0.02, -0.008, 100.0, 0.0, 100.0, true, true, false, true, 1, true)
			...
			Citizen.Wait(800)
			TriggerServerEvent("pw-smoke:eff_lighter_joint", ObjToNet(lighterjoint))
			Citizen.Wait(1000)
			...
			TriggerServerEvent("pw-smoke:eff_joint", ObjToNet(joint))
			
	  end
end)

RegisterNetEvent('pw-smoke:JointMouth')
AddEventHandler('pw-smoke:JointMouth', function(source)
	local playerPed = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(playerPed))
...
RegisterNetEvent('pw-smoke:JointHand')
AddEventHandler('pw-smoke:JointHand', function(source)
	local playerPed = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(playerPed))
...
RegisterNetEvent("pw-smoke:c_eff_lighter_joint")
AddEventHandler("pw-smoke:c_eff_lighter_joint", function(lighterjoint)
...
RegisterNetEvent("pw-smoke:c_eff_joint")
AddEventHandler("pw-smoke:c_eff_joint", function(joint)
...
RegisterNetEvent("pw-smoke:c_eff_smokes_joint")
AddEventHandler("pw-smoke:c_eff_smokes_joint", function(c_ped)
		...
		Wait(7)
		 if isSmokeJointMouth then
			...
			if IsControlJustPressed(0, Config.Smoke) then
				...
				Citizen.Wait(2200)
				TriggerServerEvent("pw-smoke:eff_smokes_joint", PedToNet(ped))
				Citizen.Wait(1200)
						noeffectjoint = false
						effect = true
							Citizen.Wait(1000)
			...
			elseif IsControlJustPressed(0, Config.Mouth) then
					TriggerEvent('pw-smoke:JointHand')
					end
				end
			end
		else
		...
		Wait(7)
		 if isSmokeJointHand then
			...
			if IsPedInAnyVehicle(playerPed, true) then
				...
				Citizen.Wait(1500)
				TriggerEvent('pw-smoke:JointMouth')
			else
				if IsEntityInWater(playerPed) then
					Citizen.Wait(800)
					SmokeDoneJoint()
