...
RegisterNetEvent('pw-smoke:CigarettesLightingAnim')
AddEventHandler('pw-smoke:CigarettesLightingAnim', function(source)
	...
	AttachEntityToEntity(light, playerPed, GetPedBoneIndex(playerPed, 4089), 0.020, -0.03, -0.010, 100.0, 0.0, 150.0, true, true, false, true, 1, true)
			...
			Citizen.Wait(800)
			TriggerServerEvent("pw-smoke:eff_lighter", ObjToNet(light))
			Citizen.Wait(1000)
			...
			DeleteObject(light)
			TriggerServerEvent("pw-smoke:eff_cigarette", ObjToNet(cigaratte))
	  end
end)

RegisterNetEvent('pw-smoke:CigarettesPackAnim')
AddEventHandler('pw-smoke:CigarettesPackAnim', function(source)
	local playerPed = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(playerPed))
...
RegisterNetEvent('pw-smoke:CigaretteMouth')
AddEventHandler('pw-smoke:CigaretteMouth', function(source)
	local playerPed = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(playerPed))
...
RegisterNetEvent('pw-smoke:CigaretteHand')
AddEventHandler('pw-smoke:CigaretteHand', function(source)
	local playerPed = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(playerPed))
...
RegisterNetEvent("pw-smoke:c_eff_lighter")
AddEventHandler("pw-smoke:c_eff_lighter", function(light)
...
RegisterNetEvent("pw-smoke:c_eff_cigarette")
AddEventHandler("pw-smoke:c_eff_cigarette", function(cigaratte)
...
RegisterNetEvent("pw-smoke:c_eff_smokes")
AddEventHandler("pw-smoke:c_eff_smokes", function(c_ped)
		...
		Citizen.Wait(7)
		 if isSmokeMouth then
			...
			if IsControlJustPressed(0, Config.Smoke) then
				...
				Citizen.Wait(2200)
				TriggerServerEvent("pw-smoke:eff_smokes", PedToNet(ped))
			...
			elseif IsControlJustPressed(0, Config.Mouth) then
					TriggerEvent('pw-smoke:CigaretteHand')
				end
			end
		else
			Wait(500)
		...
		Citizen.Wait(7)
		 if isSmokeHand then
			...
			if IsPedInAnyVehicle(playerPed, true) then
				...
				Citizen.Wait(1500)
			TriggerEvent('pw-smoke:CigaretteMouth')
			else
				if IsEntityInWater(playerPed) then
					Citizen.Wait(800)
					SmokeDone()
