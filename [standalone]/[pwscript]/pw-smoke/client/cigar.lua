...
RegisterNetEvent('pw-smoke:CigarLightingAnim')
AddEventHandler('pw-smoke:CigarLightingAnim', function(source)
	...
	AttachEntityToEntity(lighter, playerPed, GetPedBoneIndex(playerPed, 4089), 0.020, -0.03, -0.010, 100.0, 0.0, 150.0, true, true, false, true, 1, true)
		...
		playAnim('misscarsteal2peeing', 'peeing_loop', 2000)
			Citizen.Wait(800)
			TriggerServerEvent("pw-smoke:eff_lighter_cigar", ObjToNet(lighter))
			Citizen.Wait(1000)
			...
			TriggerServerEvent("pw-smoke:eff_cigar", ObjToNet(cigar))
			
	  end
end)

RegisterNetEvent('pw-smoke:CigarMouth')
AddEventHandler('pw-smoke:CigarMouth', function(source)
	local playerPed = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(playerPed))
...
RegisterNetEvent('pw-smoke:CigarHand')
AddEventHandler('pw-smoke:CigarHand', function(source)
	local playerPed = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(playerPed))
...
RegisterNetEvent("pw-smoke:c_eff_lighter_cigar")
AddEventHandler("pw-smoke:c_eff_lighter_cigar", function(lighter)
...
RegisterNetEvent("pw-smoke:c_eff_cigar")
AddEventHandler("pw-smoke:c_eff_cigar", function(cigar)
...
RegisterNetEvent("pw-smoke:c_eff_smokes_cigar")
AddEventHandler("pw-smoke:c_eff_smokes_cigar", function(c_ped)
		...
		Citizen.Wait(7)
		 if isSmokeCigarMouth then
			...
			if IsControlJustPressed(0, Config.Smoke) then
				...
				Citizen.Wait(2200)
				TriggerServerEvent("pw-smoke:eff_smokes_cigar", PedToNet(ped))
			...
			elseif IsControlJustPressed(0, Config.Mouth) then
					TriggerEvent('pw-smoke:CigarHand')
				end
			end
		else
			Wait(500)
		...
		Citizen.Wait(7)
		 if isSmokeCigarHand then
			...
			if IsPedInAnyVehicle(playerPed, true) then
				...
				Citizen.Wait(1500)
			TriggerEvent('pw-smoke:CigarMouth')
			else
				if IsEntityInWater(playerPed) then
					Citizen.Wait(800)
					SmokeDoneCigar()
