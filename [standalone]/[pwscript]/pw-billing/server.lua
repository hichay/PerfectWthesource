...
ESX.RegisterServerCallback("pw-billing:GetInvoices", function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM billing WHERE receiver_identifier = @identifier ORDER BY CASE WHEN status = "unpaid" THEN 1 WHEN status = "autopaid" THEN 2 WHEN status = "paid" THEN 3 WHEN status = "cancelled" THEN 4 END ASC, id DESC', {
...
RegisterServerEvent("pw-billing:PayInvoice")
AddEventHandler("pw-billing:PayInvoice", function(invoice_id)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM billing WHERE id = @id', {
		...
		if playerMoney < invoices.invoice_value then
			TriggerClientEvent('ESX:Notify', xPlayer.source,  "Bạn không có đủ tiền!", 'error')
		else
			xPlayer.removeAccountMoney('bank', invoices.invoice_value)
			TriggerEvent("pw-bossmenu:server:addAccountMoney", invoices.society, invoices.invoice_value)
			...
			MySQL.Async.execute('UPDATE billing SET status = @status, paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
				['@status'] = 'paid',
				['@id'] = invoice_id
			})

			TriggerClientEvent('ESX:Notify', xPlayer.source, "Hoá đơn đã được thanh toán thành công!", 'success')
...
RegisterServerEvent("pw-billing:CancelInvoice")
AddEventHandler("pw-billing:CancelInvoice", function(invoice_id)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM billing WHERE id = @id', {
		...
		MySQL.Async.execute('UPDATE billing SET status = "cancelled", paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
			['@id'] = invoice_id
		})
		TriggerClientEvent('ESX:Notify', xPlayer.source,  "Bạn đã huỷ hoá đơn này!", 'info')
		if Webhook ~= '' then
...
RegisterServerEvent("pw-billing:CreateInvoice")
AddEventHandler("pw-billing:CreateInvoice", function(data)
	local _source = ESX.GetPlayerFromId(source)
	local target = ESX.GetPlayerFromIdCard(data.target)
	--print(target)
	local webhookData = {}
	if target ~= nil then 
		MySQL.Async.fetchAll('SELECT id FROM billing WHERE id = (SELECT MAX(id) FROM billing)', {}, function(result)
			webhookData = {
				id = result[1].id + 1,
		...
		if Config.LimitDate then
			MySQL.Async.insert('INSERT INTO billing (receiver_identifier, receiver_name, author_identifier, author_name, society, society_name, item, invoice_value, status, notes, sent_date, limit_pay_date) VALUES (@receiver_identifier, @receiver_name, @author_identifier, @author_name, @society, @society_name, @item, @invoice_value, @status, @notes, CURRENT_TIMESTAMP(), DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL @limit_pay_date DAY))', {
				...
				TriggerClientEvent('ESX:Notify', target.source, "Bạn vừa nhận được một hoá đơn mới!", 'info')
				if Webhook ~= '' then
		...
		TriggerClientEvent('ESX:Notify', _source,  "Người chơi không tồn tại!", 'info')
	end
end)

ESX.RegisterServerCallback("pw-billing:GetSocietyInvoices", function(source, cb, society)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM billing WHERE society = @society ORDER BY id DESC', {
		['@society'] = society
	}, function(result)
		local invoices = {}
...
function checkTimeLeft()
	MySQL.Async.fetchAll('SELECT *, TIMESTAMPDIFF(HOUR, limit_pay_date, CURRENT_TIMESTAMP()) AS "timeLeft" FROM billing WHERE status = @status', {
		['@status'] = 'unpaid'
	}, function(result)
				...
				for k, vl in pairs(whenToAddFees) do
					if v.fees_amount == k - 1 then
						if v.timeLeft >= vl*(-1) then
							MySQL.Async.execute('UPDATE billing SET fees_amount = @fees_amount, invoice_value = @invoice_value WHERE id = @id', {
								['@fees_amount'] = k,
						...
						MySQL.Async.execute('UPDATE users SET accounts = @playerAccount WHERE identifier = @target', {
							...
							--[[  TriggerEvent("esx_addonaccount:getSharedAccount", v.society, function(account2)
								if account2 ~= nil then ]]
									...
									TriggerEvent("pw-bossmenu:server:addAccountMoney", v.society, invoice_value)
									MySQL.Async.execute('UPDATE billing SET status = @paid, paid_date = CURRENT_TIMESTAMP() WHERE id = @id', {
										['@paid'] = 'autopaid',
										['@id'] = v.id
									})
								--[[  end 
