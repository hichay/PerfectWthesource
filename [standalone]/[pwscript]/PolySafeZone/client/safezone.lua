...
function EnteredZone(isPointInside,Name)
	...
	if isPointInside and WhatIsLastLoop ~= Name then
		Citizen.CreateThread(function()
			...
			Notify('error', 'Exit')
			BlockWeapon = true
			TriggerEvent("pw-zone:chanvukhi")
...
RegisterNetEvent('pw-zone:chanvukhi')
AddEventHandler('pw-zone:chanvukhi', function()
	while BlockWeapon do 
		Citizen.Wait(0)
