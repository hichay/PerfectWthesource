...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState then
			...
			TriggerEvent("ESX:Notify","Cửa khóa","error")
        ...
        while listening do
            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 3.0 then
                    ...
                    if hasAccess and not isHidden then
						...
						--TriggerEvent("ESX:Notify",("Cửa %s"):format(newLockState and 'Khóa' or 'Mở'),newLockState and 'error' or 'success')
						showInteraction(("[E] %s"):format(newLockState and 'Mở cửa' or 'Khóa cửa'),newLockState and 'success' or 'error')
                    else
                    end
                else
                    idle = 100
...
AddEventHandler("pw-doors:doorKeyFob", function()
    ...
    if currentZone ~= nil and bollards[currentZone].inside then
        ...
        local entity = exports['bt-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
...
Citizen.CreateThread(function()
    exports["PolyZone"]:AddBoxZone("mrpd_bollards_01", vector3(411.66, -1027.95, 29.24), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    exports["PolyZone"]:AddBoxZone("mrpd_bollards_02", vector3(411.66, -1020.09, 29.34), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    TriggerServerEvent("pw-doors:request-lock-state")
end)

RegisterNetEvent("pw-doors:add")
AddEventHandler("pw-doors:add", function(pArgs, pEntity, pContext)
