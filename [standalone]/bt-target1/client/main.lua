...
function playerTargetEnable()
    ...
    while targetActive do
	    ...
	    if IsEntityAVehicle(entity) then
                for _, model in pairs(Vehicle) do
                
                        if #(plyCoords - coords) <= Vehicle[_]["distance"] then

                            success = true

                            SendNUIMessage({response = "validTarget", data = Vehicle[_]["options"]})
                            ...
                            while success and targetActive do
                                ...
                                if GetEntityType(entity) == 0 or #(plyCoords - coords) > Vehicle[_]["distance"] then
                                    success = false
                                end
...
function AddTargetModel1(models, parameteres)
    for _, model in pairs(models) do
        Vehicle[model] = parameteres
    end
end
