...
local OxyDropOffs = {
	...
	[17] =  { ['x'] = 719.73736, ['y'] = -2102.905, ['z'] = 29.24286, ['h'] = 252.19686, ['info'] = ' no1' },
	[18] =  { ['x'] = -698.2742, ['y'] = -1182.556, ['z'] = 10.711126, ['h'] = 129.96548, ['info'] = ' no2' },
	[19] =  { ['x'] = -324.5247, ['y'] = -776.9159, ['z'] = 38.779785, ['h'] = 39.336452, ['info'] = ' no3' },
...
function CreateOxyVehicle()
    ...
    while true do
    	Citizen.Wait(1)
    	 DrawText3Ds(carspawns[spawnpoint]["x"], carspawns[spawnpoint]["y"], carspawns[spawnpoint]["z"], "Xe nhập hàng.")
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			if OxyChance <= Config.OxyChance then
				TriggerServerEvent("oxydeliverymeth:receiveoxy")
			elseif OxyChance <= Config.BigRewarditemChance then
				TriggerServerEvent("oxydeliverymeth:receiveBigRewarditem")
			end

			Citizen.Wait(2000)
			--ESX:Notify('The delivery was on point, your GPS will be updated with the next drop off..', 'success')
			TriggerEvent("ESX:Notify","Toạ độ nhập hàng sẽ được cập nhật..","success")
		else
			--ESX:Notify('The drop off failed..', 'error')
			TriggerEvent("ESX:Notify","Toạ độ nhập hàng sẽ được cập nhật..","error")
		end
...
RegisterNetEvent("oxydeliverymeth:client")
AddEventHandler("oxydeliverymeth:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			--ESX:Notify('You are close to the drop off..', 'success')
			TriggerEvent("ESX:Notify","Đang đến gần tới điểm nhập..","success")
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			--ESX:Notify('You are no longer selling Oxy due to taking too long to drop off the package..', 'warning')
			TriggerEvent("ESX:Notify","You are no longer selling Oxy due to taking too long to drop off the package..","warning")
		end
		if dstcheck < 2.0 and pedCreated then

			local crds = GetEntityCoords(deliveryPed)
			DrawText3Ds(crds["x"],crds["y"],crds["z"], "[E] Nói chuyện")  
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dropOff6 < 1.6 and not OxyRun then
			...
			DrawText3Ds(pillWorker["x"],pillWorker["y"],pillWorker["z"], "Nhấn [E] để nhận") 
			if IsControlJustReleased(0,38) then
				TriggerServerEvent("oxydeliverymeth:server")
				Citizen.Wait(1000)
			end
		end
    ...
    while true do
		...
		elseif OxyRun then
			...
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				--ESX:Notify('The dealer isnt giving you anymore locations due to the state of his car..', 'error')
				TriggerEvent("ESX:Notify","Vì xe đã hư nên không nhận được vị trí giao tiếp theo..","error")
			else
				if tasking then
			        Citizen.Wait(1500)
			    else
			        TriggerEvent("oxydeliverymeth:client")  
				    salecount = salecount + 1
...
RegisterNetEvent("oxydeliverymeth:startDealing")
AddEventHandler("oxydeliverymeth:startDealing", function()
	...
	OxyRun = true
	firstdeal = true 
	--ESX:Notify('A car has been left outside for you. Your GPS will be updated with locations soon..', 'success')
	TriggerEvent("ESX:Notify","Xe ăn cắp đã được giao tới. Địa điểm lấy hàng sẽ được cập nhật ngay..","error")
	CreateOxyVehicle()
end)

local JustMakeMeth = false
local JustPackMeth = false

RegisterNetEvent('meth:spam-prevent')
AddEventHandler('meth:spam-prevent', function(prevent)
	if prevent == "makemeth" then
		Wait(900000)
		JustMakeMeth = false
	elseif prevent == "packmeth" then
		Wait(900000)
		JustPackMeth = false
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(PlayerPos, 2433.8403, 4968.9829, 42.347572, true) <= 2 then						
			
				if JustPackMeth then
					TriggerEvent("DoLongHudText","Bạn vừa mới làm một công đoạn khác rồi!")
				else			
					if not JustMakeMeth then
						TriggerEvent("weed:spam-prevent","makemeth")
						JustMakeMeth = true
					end
					if not IsPedInAnyVehicle(PlayerPedId()) and IsControlJustReleased(0,38) then
						local meththo = exports["pw-inventory"]:hasEnoughOfItem("morphine",1,false)
						...
						if meththo and water then
							local finished = exports["np-taskbar"]:taskBar(30000,"Pha chế Cocain",false,false,playerVeh)
								if (finished == 100) then
									...
									TriggerEvent("player:receiveItem","curedmeth",1)
								end
						else
							TriggerEvent("DoShortHudText","Thiếu nguyên liệu cần thiết.")
						end
					end	
				end
		...
		if GetDistanceBetweenCoords(PlayerPos, 706.96362, -966.8983, 30.412858, true) <= 2 then						
					
			if JustMakeMeth then
				TriggerEvent("DoLongHudText","Bạn vừa mới làm một công đoạn khác rồi!")
			else
				if not JustPackMeth then
					TriggerEvent("meth:spam-prevent","packmeth")
					JustPackMeth = true
				end
				if not IsPedInAnyVehicle(PlayerPedId()) and IsControlJustReleased(0,38) then
					local meththo = exports["pw-inventory"]:hasEnoughOfItem("curedmeth",1,false)
					local bag = exports["pw-inventory"]:hasEnoughOfItem("emptymethbag",1,false)
					...
					if meththo and bag and scale then
						local finished = exports["np-taskbar"]:taskBar(30000,"Đóng gói Cocain",false,false,playerVeh)
							if (finished == 100) then
								...
								TriggerEvent("player:receiveItem","methbagged",1)
							end
					else
						TriggerEvent("DoShortHudText","Thiếu nguyên liệu cần thiết.")
					end
		...
		if GetDistanceBetweenCoords(PlayerPos, 1392.5942, -2205.867, 61.462894, true) <= 5 then	
			if not IsPedInAnyVehicle(PlayerPedId()) and IsControlJustReleased(0,38) then
				...
				if meth then
					...
					TriggerEvent("DoShortHudText","Không có đủ.")
				end
			end	
		end
	end
