...
function DrugSell(ped, oldped)
    ...
    if ped ~= oldped then
		...
		TriggerServerEvent("pw-drugs:offer", ped)
	end
end
        ...
        if not IsPedDeadOrDying(ped) and not IsPedInAnyVehicle(ped) then
		    ...
		    if IsPedArmed(playerPed, 7) then
                ...
                if IsControlJustPressed(1, 306) then
				    ...
				    DeleteObject(drug)
				    TriggerServerEvent("pw-drugs:robgive", item, amount)
				    FreezeEntityPosition(ped, false)
				    ClearPedTasks(ped)
...
RegisterNetEvent("pw-drugs:startSell")
AddEventHandler("pw-drugs:startSell", function()
    if commandsell == false then
    	commandsell = true
    	selldrugs = true
    	TriggerEvent("DoLongHudText", "Bắt đầu chào gọi khách.")
    else
		if thief == false and selling == false then
			oldped = ped
			TriggerEvent("DoLongHudText", "Bạn đã ngừng bán hàng, hãy đợi 1 phút để bắt đầu lại")
			selldrugs = false
			selling = false
			sell = false
			IsSell = false
			...
			TriggerEvent("DoLongHudText", "Bạn không thể ngừng bán hàng trong sự kiện này.")
		end
	end
end)

RegisterNetEvent("pw-drugs:c_startoffers")
AddEventHandler("pw-drugs:c_startoffers", function(ped, item, price, labels, amount)
...
RegisterNetEvent("pw-drugs:canceloffers")
AddEventHandler("pw-drugs:canceloffers", function(ped)
	local playerPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(playerPed, false)
	Citizen.Wait(3000)
	IsSell = false
...
RegisterNetEvent("pw-drugs:client:sell")
AddEventHandler("pw-drugs:client:sell", function(ped)
	IsSell = true
                ...
                if DecorGetInt(ped, "NPC_ID") == 0 and not DecorExistOn(ped, "ScriptedPed") and not IsPedDeadOrDying(ped) and not IsPedInAnyVehicle(ped) then
					...
					if pedType ~= 28 and not IsPedAPlayer(ped) and not has_value(Config.BlacklistNpc, pedRelationship) then
                        ...
                        if GetGameTimer() > SellCooldown and distance <= 3 and not selling and ped ~= playerPed and ped ~= oldped or IsPedInVehicle(playerPed, vehicle, true) and GetGameTimer() > SellCooldown and distance <= 7 and not selling and ped ~= playerPed and ped ~= oldped then
                            ...
                            if IsControlJustPressed(1, 38) and GetGameTimer() > AntiSpam then
