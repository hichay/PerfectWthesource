...
RPC.register("pw-weed:getPlants", function(src)
    return weedPlants
end)

RPC.register("pw-weed:plantSeed", function(src, pCoords, pStrain)
    local pGender = 0
    local pTimestamp = os.time()

    local insertId = MySQL.insert.await([[
        INSERT INTO weed_plant (gender, coords, strain, timestamp)
        VALUES (?, ?, ?, ?)
    ]],
    ...
    TriggerClientEvent("pw-weed:trigger_zone", -1, 1, data)

    return true
end)

RPC.register("pw-weed:addWater", function(src, pPlantId)
    ...
    TriggerClientEvent("pw-weed:trigger_zone", -1, 2, weedPlants[pPlantId])

    return true
end)

RPC.register("pw-weed:addFertilizer", function(src, pPlantId, pType)
    local plant = weedPlants[pPlantId]
...
RPC.register("pw-weed:addMaleSeed", function(src, pPlantId)
    local plant = weedPlants[pPlantId]
...
RPC.register("pw-weed:removePlant", function(src, pPlantId, pFertilizer)
    local plant = weedPlants[pPlantId]
    ...
    TriggerClientEvent("pw-weed:trigger_zone", -1, 3, { id = pPlantId })

    return true
end)

RPC.register("pw-weed:harvestPlant", function(src, pPlantId)
    local plant = weedPlants[pPlantId]
        ...
        for id, plant in pairs(weedPlants) do
            if (time - plant.timestamp) >= (PlantConfig.LifeTime * 60) then
                ...
                if affectedRows and affectedRows > 0 then
                    weedPlants[id] = nil
                    TriggerClientEvent("pw-weed:trigger_zone", -1, 3, plant)
                end
            end
        end
