...
window.addEventListener('message', function(event) {
      ...
      if (item.closeGame === true) {
		...
		$("#terminal").css("background-image", "url('/html/img/bg-off.png')");
		$("#terminal").html("");
		if (Sound)
			$("#poweroff")[0].play();
      }
...
Start = function()
{
	$.get("https://breakout.bernis-hideout.de/robco-industries/ajax/wordlist.php", {
		length: Difficulty,
		count: Count
	}, WordCallback);
...
TogglePower = function()
{
	if (Power == "on")
	{
		...
		if (Sound)
			$("#poweroff")[0].play(); */
		$.post('http://fallouthack/Gamelose', JSON.stringify({
           	tasknum: 2
		})); 
	}
	else
	{
		...
		$("#terminal").css("background-image", "url('/html/img/bg.png')");
		Initialize();
	}
}
...
Failure = function()
{
	...
	},
	{
		...
		complete : function()
		{
			/* $("#terminal").html("<div id='canvas'></div><div id='adminalert'><div class='character-hover alert-text'>TERMINAL ĐÃ BỊ KHÓA</div><br />HACK VÀO HỆ THỐNG THẤT BẠI</div></div>"); */
			$("#terminal").html("<div id='canvas'></div><div id='adminalert'><div id='msg' class='character-hover alert-text'>TERMINAL ĐÃ BỊ KHÓA</div><br /><div onClick=\"LoseGame()\" id='proceed' class='alert-text'>HACK VÀO HỆ THỐNG THẤT BẠI</div></div>"); 
...
WinGame = function()
{
	$.post('http://fallouthack/Gamewin', JSON.stringify({
           tasknum: 3
	})); 
}
...
Success = function()
{
	...
	},
	{
		...
		complete : function()
		{
			...
			$("#terminal").html("<div id='canvas'></div><div id='adminalert'><div id='msg' class='character-hover alert-text'>ĐẪ HACK VÀO HỆ THỐNG THÀNH CÔNG</div><br /><div onClick=\"WinGame()\" id='proceed' class='alert-text'>PRESS HERE TO PROCEED</div></div>"); 
