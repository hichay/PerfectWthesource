...
(function () {
  ...
  function closeMain() {
		...
		$.post('http://fishingame/taskEnd', JSON.stringify({
              tasknum: 3
         }));
	...
	function openMain() {
		TweenMax.set(".game", { display: 'flex' });
		resetGame()
		this.okjel = 10
  ...
  window.addEventListener('message', function(event) {
      ...
      if (item.runProgress === true) {
		  /* console.log('Rest the game') */
          TweenMax.set(".game", { display: 'flex' });
		  resetGame()
		  fishspeedd = item.Speedfish
      }
  ...
  class Fish {
    ...
    updateFishPosition() {
      if (!this.randomPosition || this.randomCountdown < 0) {
        ...
        this.speed = Math.abs(Math.random() * (3 - 1) + fishspeedd);
      };
  ...
  class ProgressBar {
    ...
    fill() {
      if (this.progress < 100) this.progress += 0.4;
    }
    ...
    detectGameEnd() {
      if (this.progress >= 100) {
		...
		// this is win game then close the game 
		// closeMain()
		$.post('http://fishingame/taskEnd', JSON.stringify({
              tasknum: 3
          }));
	   ...
	   if (this.progress == 0) {
		 ...
		 $.post('http://fishingame/taskCancel', JSON.stringify({
           tasknum: 2
		})); 
  ...
  successButton.addEventListener('click', closeMain);
