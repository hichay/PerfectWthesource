...
Citizen.CreateThread(function()
	...
	local query = '`id`, `accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`' -- Select these fields from the database
	...
	if Config.Multichar then -- insert identity data with creation
		...
		MySQL.Async.store("INSERT INTO `users` SET `accounts` = ?, `identifier` = ?, `license` = ?, `group` = ?", function(storeId)
			NewPlayer = storeId
		end)
	end
		...
		if ESX.GetPlayerFromIdentifier(identifier) then
			...
			MySQL.Async.fetchScalar('SELECT 1 FROM users WHERE identifier = @identifier', {
				...
				if result then
					...
					-- createESXPlayer(identifier, license, playerId) 
					local accounts = {}
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll(LoadPlayer, { identifier
			...
			--Pernament Id
			if result[1].id then 
				userData.id = tonumber(result[1].id)
			end
	...
	Async.parallel(tasks, function(results)
		local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.loadout, userData.playerName, userData.coords, userData.id)
		ESX.Players[playerId] = xPlayer
		ESX.PlayersID[tonumber(userData.id)] = xPlayer
