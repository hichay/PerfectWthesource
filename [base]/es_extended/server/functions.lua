				...
				if not error and command.suggestion.arguments then
					...
					for k,v in ipairs(command.suggestion.arguments) do
						if v.type then
							...
							elseif v.type == 'idcard' then
									local targetPlayer = tonumber(args[k])

									if args[k] == 'me' then targetPlayer = playerId end

									if targetPlayer then
										local xTargetPlayer = ESX.GetPlayerFromIdCard(targetPlayer)
...
Citizen.CreateThread(function()
	savePlayers = MySQL.prepare("UPDATE users SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ? WHERE `identifier` = ?")
end)
...
ESX.GetPlayerFromIdCard = function(id)
	return ESX.PlayersID[tonumber(id)]
end
