...
function ESX.Notify( message, type)
	local title = ""
    local ttype = type ~= nil and type or "info"
    local time = 5000
	SendNUIMessage({
		action = 'open',
		title = title,
...
AddEventHandler('ESX:Notify', function(message, type)
	ESX.Notify( message, type)
end)

RegisterNetEvent('ESX:ShowInfo')
AddEventHandler('ESX:ShowInfo', function(message, type)
	ESX.ShowInfo( message, type)
end)

function ESX.ShowInfo( message, type)
	local title = ""
    local ttype = type ~= nil and type or "info"
    local time = 10000
	SendNUIMessage({
...
ESX.ShowAdvancedNotification = function(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
	...
	AddTextEntry('esxAdvancedNotification', "<FONT FACE='Reaver-SemiBold'><b>"..msg.."</b></FONT>")
	BeginTextCommandThefeedPost('esxAdvancedNotification')
	if hudColorIndex then ThefeedNextPostBackgroundColor(hudColorIndex) end
	EndTextCommandThefeedPostMessagetext(textureDict, textureDict, false, iconType, sender, "<FONT FACE='Reaver-SemiBold'><b>"..subject.."</b></FONT>")	
...
ESX.ShowHelpNotification = function(msg, thisFrame, beep, duration)
	...
	if string.match(msg, "~INPUT_SCRIPTED_FLY_ZUP~") then
        msg = msg:gsub("~INPUT_SCRIPTED_FLY_ZUP~", "[PageUp]")
    end
	
	if string.match(msg, "~INPUT_SCRIPTED_FLY_ZDOWN~") then
        msg = msg:gsub("~INPUT_SCRIPTED_FLY_ZDOWN~", "[PageDown]")
    end
	
	AddTextEntry('esxHelpNotification', "<FONT FACE='Reaver-SemiBold'><b>"..msg.."</b></FONT>")
...
ESX.Game.SpawnVehicleqb = function(model, cb, coords, isnetworked)
    local model = (type(model)=="number" and model or GetHashKey(model))
    local coords = coords ~= nil and coords or ESX.Game.GetCoordsqb(GetPlayerPed(-1))
...
Citizen.CreateThread(function()
	...
	RegisterFontFile("fontnew1")
	RegisterFontId("Oswald")	
	AddTextEntry('CUSTOM_TEXT1', "<FONT FACE='Oswald'>~a~</FONT>")
