...
onNet("insideShell", (inside: boolean) => {
    if (inside === true) {
        ...
        weatherFrozen = false;
        emitNet("pw-weathersync:client:weather:request");
    }
})
...
const setWeather = async (weather: WeatherProgression, skipFreeze = false): Promise<void> => {
    ...
    if (currentWeather.weather !== weather.weather) {
        // emit("pw-weathersync:currentWeather", weather.weather)
...
onNet("pw-weathersync:client:weather", async (weather: WeatherProgression) => {
