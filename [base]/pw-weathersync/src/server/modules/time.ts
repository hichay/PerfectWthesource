...
setImmediate(() => {
    ...
    emitNet("pw-weathersync:client:time", -1, time)
})
...
setInterval(() => {
    ...
    emit("pw-weathersync:server:time", time)
    emitNet("pw-weathersync:currentTime", -1, Math.floor(time / 60), time % 60)
}, secondsPerMinute * 1000)

onNet("pw-weathersync:setTime", (source: string, param: string) => {
...
onNet("pw-weathersync:client:time:request", () => {
    emitNet("pw-weathersync:client:time", global.source, time)
})
