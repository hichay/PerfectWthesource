...
function addPlayerToRadio(plySource)
	...
	if radioPressed then
		print('[radio] %s joined radio %s while we were talking, adding them to targets', plySource, radioChannel)
		...
		print('[radio] %s joined radio %s', plySource, radioChannel)
	end
end
...
function removePlayerFromRadio(plySource)
	if plySource == playerServerId then
		...
		if radioPressed then
			...
			print('[radio] %s left radio %s while we were talking, updating targets.', plySource, radioChannel)
			playerTargets(radioData, NetworkIsPlayerTalking(PlayerId()) and callData or {})
		else
			logger.info('[radio] %s has left radio %s', plySource, radioChannel)
			print('[radio] %s has left radio %s', plySource, radioChannel)
		end
	end
end
...
RegisterCommand('+radiotalk', function()
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			print('talking on radio channel')
			playerTargets(radioData, NetworkIsPlayerTalking(PlayerId()) and callData or {})
