    ...
    if (event.data.type == "updateVehicleInfos") {
        ...
        $('#contentVehicle').append(`
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Điều khiển</span>
                        <div class="bar">
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Tốc độ tối đa</span>
                        <div class="bar">
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Tăng tốc</span>
                        <div class="bar">
                    ...
                    <div class="row" id="actionButton">
                        <a class="buyButton" style=" color:#FC1259;" onclick="openModalMenu()"> Mua xe </a>
                    </div>
                `);

                if (isEnableTestDrive) {
                    $('#actionButton').append(`
                        <a class="tdButtton" onclick="testDrive()"> Lái thử </a>
                    `);
                }
    }
...
function menuVehicle(event) {
    ...
    $.post('http://pw-vehicleshop/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('http://pw-vehicleshop/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));    
...
function openModalMenu(){    
    ...
    $('#closemenu').append(`
        ...
        <div class="modal-content">
            <p class="title">Xác nhận mua:</p>
            <p class="vehicle">Xe cộ</p>         

            <p>Hãng: <span class="brand">`+vehicleSelected.brand+`</span></p>
            <p>Loại: <span class="model">`+vehicleSelected.modelcar+`</span></p>
        </div>

        <div class="modal-footer">
            <div class="modal-price">
                <p class='price sale'>$ `+ vehicleSelected.sale/1000+` k</p>
                <p class='price discount'>$ `+ parseFloat(vehicleSelected.sale/1000 * 0).toFixed(3)+` k</p>         
            </div>
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('http://pw-vehicleshop/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $.post('http://pw-vehicleshop/Close');
            $('body').css("display","none");
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('http://pw-vehicleshop/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('http://pw-vehicleshop/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('http://pw-vehicleshop/rotate', JSON.stringify({key: _key}))
    }
    ...
    switch(event.which) {        
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                //vehicleSelected = {brand: dataCar[0].outerText, modelcar: dataCar[9].outerText, sale: dataCar[7].outerText/3000, name: dataCar[1].outerText }
                ...
                $.post("http://pw-vehicleshop/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));
...
/* function openOption(option){
 ...
 function setVehicleColorRGB(R,G,B){
     ...
     if (selectedColor == 'primary'){
         $.post("http://pw-vehicleshop/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
     } else {
         $.post("http://pw-vehicleshop/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
     }
