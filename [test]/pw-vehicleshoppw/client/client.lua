    ...
    function()
        while true do
            ...
            local dist = #(actualShop - GetEntityCoords(ped))
                if dist <= 50.0 then                    
                    if dist <= 4.0 then                 
                        DrawText3Ds(actualShop.x, actualShop.y, actualShop.z,"Nhấn ~r~E~w~ để mở shop xe")
                    end
...
RegisterNetEvent('vehicleshop.basarili')
AddEventHandler('vehicleshop.basarili', function(vehicleName,vehiclePlate,value)    
    SendNUIMessage(
        {
            type = "sussess-buy",
...
AddEventHandler('vehicleshop.vehiclesInfos', function(tableVehicles)     
    --print(json.encode(tableVehicles)) 
    ...
    for _,value in pairs(tableVehicles) do
        ...
        local brandName
         ...
         -- if Config.Build2060 then
	      ...
	      -- brandName = GetMakeNameFromVehicleModel(vehicleModel)    
         -- else
            -- brandName = nil	
			-- --print(GetMakeNameFromVehicleModel(vehicleModel))
         -- end

       
		
		if value.brand == "NULL" then 
			brandName = 'Custom'
		else
			--brandName = 'Custom'
			brandName = GetLabelText(value.brand)
		end
...
function OpenVehicleShop()
    ...
    while inTheShop do
        Citizen.Wait(0)        
		DrawSpotLightWithShadow(-37.26872,-1054.309,-43.37314, 255, 255, 255, 20.000, 100.000)
    end
...
AddEventHandler('vehicleshop.spawnVehicle', function(model, plate)    
	...
	-- TriggerEvent("vehiclekeys:client:SetOwner", ESX.Game.GetVehicleProperties(vehicleBuy).plate)
 ...
 RegisterNUICallback(
     ...
     function(data, cb)        
         if Config.TestDrive then
			 ...
			 TriggerEvent("vehiclekeys:client:SetOwner", ESX.Game.GetVehicleProperties(testDriveEntity).plate)
             ...
             while startCountDown do
                 ...
                 if GetGameTimer() < timeGG+tonumber(1000*Config.TestDriveTime) then
                     ...
                     drawTxt("lái thử" .. math.ceil(Config.TestDriveTime - secondsLeft/1000),4,0.5,0.93,0.50,255,255,255,180)
                 else
                    DeleteEntity(testDriveEntity)
					TriggerServerEvent('shop:removeVehicle',"TEST")
...
Citizen.CreateThread(function ()
    ...
    for i = 1, #vehicleshopCoords do    
        ...
        AddTextComponentString('Cửa hàng xe')
