...
Graffiti.Register = function(art) 
...
Graffiti.SelectArtwork = function(allow_overwrite,cb)
  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "graffiti_menu",{
      title    = "Select Graffiti",
      align    = 'left',
      elements = elements,
    }, 
    function(data,menu)
      menu.close()
      if data.current.type == "new" then
        cb(data.current.value) 
      elseif data.current.type == "overwrite" then
        cb(data.current.value,true)
      elseif data.current.type == "custom" then
...
Graffiti.Overwrite = function(artwork,modifier)
  local closest,dist = Graffiti.GetClosest()
  if closest and dist <= 20.0 then
...
Citizen.CreateThread(Graffiti.Init)
