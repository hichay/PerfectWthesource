...
RegisterCommand("getmod", function(source, args, rawCommand)
    local plyPed = PlayerPedId()
    local veh = GetVehiclePedIsIn(plyPed, false)
    local vehicleMods = ESX.Game.GetVehicleProperties(veh)
...
function RepairVehicle()
    Citizen.CreateThread(function ()
        ...
        if missingEngineHealth > 50 or missingBodyHealth > 50 then
            local finished = exports["pw-taskbar"]:taskBar(5000 + (missingEngineHealth / 50), "Sửa chữa...", true, nil, nil, nil, nil, 6.0)
        ...
        --[[ if missingBodyHealth > 50 then
            local finished = exports["pw-taskbar"]:taskBar(5000 + (missingBodyHealth / 50), "Repairing body...", true, nil, nil, nil, nil, 6.0)
        ...
        TriggerServerEvent("pw-vehicles:bennysResetDegradation",GetVehicleNumberPlateText(plyVeh))
    end)
...
function ApplyMod(categoryID, modID)
    ...
    elseif categoryID == 11 or categoryID == 12 or categoryID== 13 or categoryID == 15 or categoryID == 16 then --Performance Upgrades
        ...
        SetVehicleMod(plyVeh, categoryID, modID)
    elseif categoryID == 48 then
        originalCategory = categoryID
        originalMod = modID

        SetVehicleMod(plyVeh, categoryID, modID)
        SetVehicleLivery(plyVeh,tonumber(modID))
        print('lvie')
    else 
        originalCategory = categoryID
        originalMod = modID
...
AddEventHandler("bennys:enter", function()
    ...
    if bennysAccess then
        ...
        local govjob = ESX.PlayerData.job.name
        if govjob == "doc" or govjob == "ambulance" then
            govjob = "police"
        end
