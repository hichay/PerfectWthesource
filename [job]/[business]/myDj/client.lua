...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k, v in pairs(Config.DJPositions) do
            if (not Config.useESX or v.requiredJob == nil or ESX ~= nil and ESX.PlayerData.job ~= nil) then
...
Citizen.CreateThread(function()
    while true do
        ...
        if isAtDJ then
            ...
            if IsControlJustReleased(0, 38) and exports["pw-inventory"]:hasEnoughOfItem(currentDJ.requiredJob,1) then
                -- open DJ Interface
                SetNuiFocus(true, true)
                isDjOpen = true
