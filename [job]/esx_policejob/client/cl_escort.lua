...
function policeEscort()
	...
	if not inmenus and job == 'police' then
		...
		if isInVeh and job == 'police' then
			TriggerEvent("startSpeedo")
		else
			TriggerEvent("pw-police:escort")
		end
	end
end
...
AddEventHandler("pw-police:escort", function()
	if exports["pw-lib"]:getVar("handcuffed") and DecorGetInt(PlayerPedId(), "escorting") == 0 and DecorGetInt(PlayerPedId(), "escorted") == 0 and DecorGetInt(PlayerPedId(), "dragging") == 0 and DecorGetInt(PlayerPedId(), "dragged") == 0 and not IsPedRagdoll(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPlayerFreeAiming(PlayerId()) then
		...
		if DecorGetInt(PlayerPedId(), "escorting") ~= 0 then
			TriggerServerEvent("pw-police:escort", DecorGetInt(PlayerPedId(), "escorting"), 0, false)
			DecorSetInt(PlayerPedId(), "escorting", 0)
		...
		if DecorGetInt(PlayerPedId(), "dragging") ~= 0 then
			TriggerServerEvent("pw-police:escort", DecorGetInt(PlayerPedId(), "dragging"), 0, true)
...
RegisterNetEvent("pw-police:escortReceive")
AddEventHandler("pw-police:escortReceive", function(pToggle, pDrag)
	if pDrag then
...
RegisterNetEvent("pw-police:escortingReceive")
AddEventHandler("pw-police:escortingReceive", function(pToggle, pDrag)
	if pDrag then
	...
	if DecorGetInt(PlayerPedId(), "escorting") == 0 and DecorGetInt(PlayerPedId(), "escorted") == 0 and DecorGetInt(PlayerPedId(), "dragging") == 0 and DecorGetInt(PlayerPedId(), "dragged") == 0 and not IsPedRagdoll(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPlayerFreeAiming(PlayerId()) then
		...
		if distance ~= -1 and distance < 1.0 and GetEntitySpeed(ped) < 1.0 and not IsPedInAnyVehicle(ped, false) then
			if DecorExistOn(ped, "dragged") and DecorGetInt(ped, "dragged") == 0 then
				...
				TriggerServerEvent("pw-police:escort", GetPlayerServerId(t), -1, true)
			end
		end
	else
		if DecorGetInt(PlayerPedId(), "escorting") ~= 0 then
...
RegisterNetEvent("pw-police:resetEscort")
AddEventHandler("pw-police:resetEscort", function()
	if DecorGetInt(PlayerPedId(), "escorting") ~= 0 then
		TriggerServerEvent("pw-police:escort", DecorGetInt(PlayerPedId(), "escorting"), 0, false)
		DecorSetInt(PlayerPedId(), "escorting", 0)
		ClearPedTasksImmediately(PlayerPedId())
...
AddEventHandler("police:forceEnter", function()
	...
	if isInVeh then
		...
		local veh = exports["pw-interact"]:GetCurrentEntity()

		if not IsEntityAVehicle(veh) then
			TriggerEvent("DoLongHudText", "Hãy nhìn vào phương tiện muốn áp giải", 2)
			return
		end

		if GetVehicleEngineHealth(veh) < 100.0 then
			TriggerEvent("DoLongHudText", "Xe này đã bị hư hỏng nặng!", 2)
			return
		end

		if not AreAnyVehicleSeatsFree(veh) then
			TriggerEvent("DoLongHudText", "Không còn ghế trống nào!", 2)
			return
		end
...
AddEventHandler("unseatPlayer", function()
	...
	if not IsEntityAVehicle(veh) then
		TriggerEvent("DoLongHudText", "Hãy nhìn vào xe!!", 2)
		return
	end
	...
	if seat then
		...
		if tPly < 0 then
			TriggerEvent("DoLongHudText", "Không có người nào!", 1)
			return
		end
		...
		if DecorGetInt(PlayerPedId(), "escorting") == 0 then
			...
			TriggerServerEvent("pw-police:escort", tServerId, -1, false)
		end
	else
		TriggerEvent("DoLongHudText", "Không có ai trong chiếc xe này", 1)
	end
end)
...
Citizen.CreateThread(function()
	...
	exports["pw-keybinds"]:registerKeyMapping("", "Police", "Escort / Speedo", "+policeEscort", "-policeEscort", "LEFT")

    RegisterCommand("+policeSeat", policeSeat, false)
	RegisterCommand("-policeSeat", function() end, false)
	exports["pw-keybinds"]:registerKeyMapping("", "Police", "Force into Vehicle", "+policeSeat", "-policeSeat", "RIGHT")
end)
