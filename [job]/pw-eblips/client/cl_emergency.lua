...
function GetBlipSettings(pJobId, pCallSign, pDepartment)
	local settings = {}
...
function CreateBlipHandler(pServerId, pJob, pCallSign, pDepartment)
	...
	local settings = GetBlipSettings(job, callsign, pDepartment)
...
AddEventHandler("esx:setJob", function(job)
	...
	elseif jobn == "ambulance" then
		DecorSetInt(PlayerPedId(), "EmergencyType", 2)
	elseif jobn == "doc" then
...
AddEventHandler('esx:onPlayerLogout', function(player)
	if BlipHandlers[player] then
