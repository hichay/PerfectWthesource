...
ESX.RegisterServerCallback('esx_ambulancejob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `state` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			['@owner'] = xPlayer.identifier,
			['@plate'] = foundPlate,
			['@job'] = xPlayer.job.name
		}, function (rowsChanged)
	...
	for j = 1, #data do
		...
		MySQL.Async.fetchAll('SELECT * FROM users WHERE is_dead = @is_dead AND identifier = @identifier' , {
			...
			for i = 1, #result do 
				...
				table.insert(cacheName, {
					firstname = result[i].firstname,
					lastname = result[i].lastname,
					status = result[i].reciverstatus,
					playeraccepted = result[i].recivername
...
AddEventHandler("esx_ambulancejob:setReciverStatus", function(iddeadplayer)
	...
	--print("id người chết.." ..deadplayerid)
		...
		playerAccepted = GetPlayerName(_source)
