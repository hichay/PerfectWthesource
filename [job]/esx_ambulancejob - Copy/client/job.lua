	...
	ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
		if quantity > 0 then
			...
			if IsPedDeadOrDying(closestPlayerPed, 1) then
				...
				TriggerServerEvent('esx_billingmk:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'Tiền viện phí', Config.InvoicePrice)
			else
...
function deadPlayerMenu()
	...
	ESX.TriggerServerCallback('esx_ambulancejob:getPlayerName', function(result)
	   ...
	   --Nameplayer = result
		function Accept(id)
            return "<a id='button' btn-type='accept' btn-plate='" .. plate .. "' href=\"#\" data-toggle=\"modal\" data-target=\"#payModal\" class=\"btn btn-warning btn-icon-split\"><span class=\"icon text-white-50\"> <i class=\"fas fa-money-check-alt\"></i></span><span class=\"text\">Trả tiền</span></a>"
        end
		...
		for k, v in ipairs(result) do 
			for _, player in ipairs(GetActivePlayers()) do
				if IsPlayerDead(player) then 
					...
					table.insert(Nameplayer,{name = v.firstname .." | " ..v.lastname, status = v.status, id = serverId, distance = distance, PlayerAccepted = v.playeraccepted, NameAccepted = PlayerNameAccepted ,Accept(serverId)})
		...
		--[[ for k,v in pairs(Nameplayer) do 
