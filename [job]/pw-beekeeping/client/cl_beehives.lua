...
AddEventHandler("np-beekeeping:removeHive", function(data)
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["pw-taskbar"]:taskBar(10, "Removing", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getReady = getHiveReadyPercent(getHiveById(data.id)) > 20.0
        local success = RPC.execute("np-beekeeping:removeHive", data.id, getReady)
        if not success then
            print("Could not remove. pid:", data.id)
        end
    end
end)
...
AddEventHandler("pw-beekeeping:harvestHive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    ...
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["pw-taskbar"]:taskBar(5000, "Harvesting", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
