...
$(function() {
    ...
    window.addEventListener("message", function(t) {
        ...
        if ("ui" === i.type) 1 == i.status ? a(!0) : a(!1);
            ...
            let e, a = i.banlist;
            $.each(a, function(a, t) {
                e = 0 == t.time ? "Vô thời hạn" : t.time < 0 ? "Hết hạn" : "Trong " + t.time + " phút", $("#banlist").append(`<div class = "banitem" data-license=${t.license}><span class="bannedplayer">${t.name}</span><span class="time">${e}</span><span class="reason">${t.reason}</span></div>`)
            })
            ...
            }), $("#vehiclelist").html(""), $("#vehiclelist").html('<div id="inventoryitemwrap"><div class = "inventoryitem" data-vehiclename="blank"><div class="img"><i class="fas fa-trash-alt centered" style="color:#ffffff;font-size:48px;"></i></div><div class = "name">Xóa xe</div></div></div>');
            let s = i.vehiclelist;
            $.each(s, function(e, a) {
        ...
        $.post("https://pw-adminpanel/" + a, JSON.stringify({
            playerid: id,
            inputData: e
        })), i(!0)
        ...
        var t, n, s;
        t = "unban", n = e, s = "Có chắc muốn unban ? " + a + "?", i(), $("#confirmaction").attr("action", t).attr("data", n).slideDown(), $("#confirmaction #inputtitle").html(s)
    }), $("#items").on("click", ".inventoryitem", function() {
        let e = $(this).data("itemname"),
            a = parseInt($("#qty").val());
        $.post("https://pw-adminpanel/giveitem", JSON.stringify({
            playerid: id,
            name: e,
            amount: a
        }))
        ...
        $.post("https://pw-adminpanel/weapon", JSON.stringify({
            playerid: id,
            weapon: e
        }))
    }), $("#vehicles").on("click", ".inventoryitem", function() {
        let e = $(this).data("vehiclename");
        $.post("https://pw-adminpanel/spawnvehicle", JSON.stringify({
            model: e
        }))
        ...
        switch (s) {
            case "kick":
                t(a = "Kick " + playerData[id].name + " với lý do", e = "reason", n = "text", s);
                break;
            case "addCash":
                t(a = "Cho tiền " + playerData[id].name, e = "100$", n = "number", s);
                break;
            case "addBank":
                t(a = "Cho tiền vào bank " + playerData[id].name, e = "100$", n = "number", s);
                break;
            case "announce":
                t(a = "Thông báo", e = "message", n = "text", s);
                break;
            case "promote":
                let o = $(this).data("level");
                $.post("https://pw-adminpanel/promote", JSON.stringify({
                    playerid: id,
                    level: o
                }));
                break;
            ...
            case "spawnVehicle":
                ...
                break;
            case "inventory":
                $.post("https://pw-adminpanel/inventory", JSON.stringify({
                    playerid: id
                }));
                break;
            case "ban":
                t(a = "Đặt thời gian (tính theo phút)", e = "100", n = "number", s);
                break;
            case "permaban":
                t(a = "Vô thời hạn " + playerData[id].name + " với lý do", e = "reason", n = "text", s);
                break;
            case "banlist":
                i(!1, "#bans", "#server");
                break;
            case "setJob":
                ...
                $.post("https://pw-adminpanel/setJob", JSON.stringify({
                    playerid: id,
                    job: r,
                    rank: l
                }));
                break;
            case "setTime":
                t(a = "Đổi thời gian trong game <br /> (24 giờ)", e = "12:00", n = "time", s);
                break;
            case "changeWeather":
                let d = $("select#weatherTypes option").filter(":selected").val();
                $.post("https://pw-adminpanel/changeWeather", JSON.stringify({
                    playerid: id,
                    weather: d
                }));
                break;
            default:
                $.post("https://pw-adminpanel/" + s, JSON.stringify({
                    playerid: id
                }))
        }
    ...
    $("#inner").append(atob("PGZvb3Rlcj4mY29weTtTb2xhclNjcmlwdHMgQWRtaW4gUGFuZWwgLSBzb2xhcnNjcmlwdHMuc3RvcmU8L2Zvb3Rlcj4=")), document.onkeyup = function(e) {
        if (27 == e.which) return $.post("https://pw-adminpanel/exit", JSON.stringify({})), void a(!1)
    }, $("#tpwp-button").click(function() {
        $.post("https://pw-adminpanel/tp-wp")
