...
RegisterNUICallback("ban", function(data)
    TriggerServerEvent("PW:Ban", data.playerid, tonumber(data.inputData), "You have been put on a timeout")
end)

RegisterNUICallback("permaban", function(data)
    TriggerServerEvent("PW:Ban", data.playerid, 0, data.inputData)
end)

RegisterNUICallback("unban", function(data)
    TriggerServerEvent("PW:Unban", data.confirmoutput)
    PW.GetPlayers()
end)

RegisterNUICallback("addCash", function(data)
    local amnt = tonumber(data.inputData)
    TriggerServerEvent("PW:AddCash", data.playerid, amnt)
end)

RegisterNUICallback("addBank", function(data)
    local amnt = tonumber(data.inputData)
    TriggerServerEvent("PW:AddBank", data.playerid, amnt)
end)
...
RegisterNUICallback("giveitem", function(data)
    ...
    TriggerServerEvent("PW:AddItem", data.playerid, data.name, amnt)
end)
...
RegisterNUICallback("tp-wp", function(data)
    PW.TeleportToWaypoint()
end)

RegisterNUICallback("bring", function(data)
    TriggerServerEvent("PW:Teleport", data.playerid, "bring")
end)

RegisterNUICallback("goto", function(data)
    TriggerServerEvent("PW:Teleport", data.playerid, "goto")
end)

RegisterNUICallback("kick", function(data)
    TriggerServerEvent("PW:Kick", data.playerid, data.inputData)
end)

RegisterNUICallback("spectate", function(data)
	playerID = data.playerid
	PW.Spectate(playerID, true)
	isSpectating = true
	SetDisplay(false)
end)

RegisterNUICallback("freeze", function(data)
	TriggerServerEvent("PW:Freeze", data.playerid)
end)

RegisterNUICallback("kill", function(data)
	TriggerServerEvent("PW:Slay", data.playerid)
end)

RegisterNUICallback("promote", function(data)
	TriggerServerEvent("PW:Promote", data.playerid, data.level)
end)

RegisterNUICallback("weapon", function(data)
	TriggerServerEvent("PW:GiveWeapon", data.playerid, data.weapon)
end)
...
RegisterNUICallback("god", function(data)
	TriggerServerEvent("PW:God", data.playerid)
end)

RegisterNUICallback("spawnvehicle", function(data)
	PW.SpawnVehicle(data.model)
end)

RegisterNUICallback("announce", function(data)
	TriggerServerEvent("PW:Announcement", data.inputData)
end)

RegisterNUICallback("setJob", function(data)
	TriggerServerEvent("PW:setJob", data.playerid, data.job, data.rank)
end)

RegisterNUICallback("revive", function(data)
	TriggerServerEvent("PW:revive", data.playerid)
end)

RegisterNUICallback("setTime", function(data)
	TriggerServerEvent("PW:Time", data.inputData)
end)

RegisterNUICallback("freezeTime", function(data)
	TriggerServerEvent("PW:freezeTime")
end)

RegisterNUICallback("changeWeather", function(data)
	TriggerServerEvent("PW:Weather", data.weather)
end)

RegisterNUICallback("freezeWeather", function(data)
	TriggerServerEvent("PW:freezeWeather")
end)

RegisterNUICallback("blackout", function(data)
	TriggerServerEvent("PW:Blackout")
end)
...
PW.TeleportToWaypoint = function()
    ESX.TriggerServerCallback("esx_marker:fetchUserRank", function(playerRank)
        if Config.Perms[playerRank] and Config.Perms[playerRank].CanTpWp then
            ...
            TriggerEvent('chat:addMessage', {args = {"PW", " You do not have permission for this."}})
        end
    end)
end

PW.GetPlayers = function()
    ESX.TriggerServerCallback("PW:getPlayers", function(players) 
        SendNUIMessage({type = "data", data = players})
    end)

	ESX.TriggerServerCallback("PW:getBanList", function(bans) 
	    SendNUIMessage({type = "bans", banlist = bans})
	end)
end

PW.GetItemList = function()
	local weapons = ESX.GetWeaponList()
	ESX.TriggerServerCallback("PW:getJobs", function(jobs) 
	    ESX.TriggerServerCallback("PW:getItemList", function(results) 
...
AddEventHandler('PW:Freeze', function(targetPed)
...
AddEventHandler('PW:Slay', function(targetPed)
	SetEntityHealth(PlayerPedId(), 0)
end)

local hasGodmode = false
RegisterNetEvent('PW:God')
AddEventHandler('PW:God', function(targetPed)
...
PW.SpawnVehicle = function(model)
    ESX.TriggerServerCallback("esx_marker:fetchUserRank", function(playerRank)
...
PW.Spectate = function(target, bool)
    ESX.TriggerServerCallback("esx_marker:fetchUserRank", function(playerRank)
...
PW.Noclip = function()
    ESX.TriggerServerCallback("esx_marker:fetchUserRank", function(playerRank)
        if Config.Perms[playerRank] and Config.Perms[playerRank].CanNoClip then
			...
			TriggerEvent('chat:addMessage', {args = {"PW ", " Noclip has been " .. msg}})
...
RegisterCommand("admin", function(source,args)
	ESX.TriggerServerCallback("esx_marker:fetchUserRank", function(playerRank)
        if Config.Perms[playerRank] then
    		...
    		TriggerEvent('chat:addMessage', {args = {"PW", " You do not have permissions for this"}})
    	end
    end)
end)
