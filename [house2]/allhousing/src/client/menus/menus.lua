...
OpenInventory = function()
  if Config.UsingESX then
    if not Config.UsingDiscInventory then
      ...
      -- end,InsideHouse)  
	  -- print(InsideHouse)
	  -- this is for new inventory
	   TriggerEvent("server-inventory-open", "1", 'housing-' .. InsideHouse.Id)
    else
...
BreakInHouse = function(d)  
  ...
  if Config.LockpickRequired then
    ...
    --if not hasItem then
      local hasItem = exports["pw-inventory"]:hasEnoughOfItem("lockpick",1,false)
      if not hasItem then
  ...
  if Config.UsingLockpickV1 then
    local finished = exports["np-lockpicking"]:lockpick(1,1,1,1)	
