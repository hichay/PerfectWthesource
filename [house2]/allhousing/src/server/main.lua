...
PurchaseHouse = function(house)
  ...
  if not v.Owned then
    ...
    if afford then
      ...
      TriggerEvent('bb-garages:server:buyHouseGarage', v.Id, v.Owner, v.Entry , v.OwnerName)
  ...
  if Houses and type(Houses) == "table" then
    for k1,v1 in pairs(Houses) do
      ...
      if not foundMatch then
		...
		newHouses[HouseKey].ShellGarage = v1.ShellGarage
        ...
        newHouses[curInDb[foundMatch].id].Shells = (curInDb[foundMatch].shell ~= "0" and curInDb[foundMatch].shell or false)
		newHouses[curInDb[foundMatch].id].ShellGarage = (curInDb[foundMatch].ShellGarage ~= "0" and curInDb[foundMatch].ShellGarage or false)
    ...
    for k1,v1 in pairs(curInDb) do
      ...
      if not foundMatch then
        newHouses[v1.id] = {
		  ...
		  ShellGarage = (v1.ShellGarage ~= "0" and v1.ShellGarage or false),
