...
AddEventHandler('qb-trunk:client:KidnapTrunk', function()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
            ...
            ESX.ShowNotification('You didn\'t kidnap this person!')
        end
    end
end)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            -- if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                ESX.ShowNotification('You are in the trunk.')
                                TrunkCam(true)
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                ESX.ShowNotification('Is the trunk closed?')
                            end
                        end
                    else
                        ESX.ShowNotification('Anyone in there yet?')
                    end 
                else
                    ESX.ShowNotification('You are already in the trunk')
                end 
            else
                ESX.ShowNotification('You cannot put this vehicle in the trunk..')
            end
        end, plate)
    else
        ESX.ShowNotification('You cannot put this vehicle in the trunk..')
...
AddEventHandler('qb-trunk:client:GetIn', function(isKidnapped)
    ...
    if closestVehicle ~= 0 then
        ...
        ESX.ShowNotification('No vehicle in sight..')
    end
end)
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            ESX.ShowNotification('Is the trunk open?')
                        end
                    end
