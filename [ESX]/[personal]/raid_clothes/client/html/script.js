    ...
    window.addEventListener('message', function (event) {
        ...
        if (event.data.type == "enableclothing_shop") {
            ...
            if (open) {
                ...
                currentRank = event.data.currentRank;
                    ...
                    if(currentRank !== 'admin' && ele.is(inputs.eq(0)) && (whitelisted[gender][clothingName] || hiddenItems[gender][clothingName])) {
                        while (isNotValid) {
                            ...
                            (
                                (
                                    !isService &&
                                    whitelisted[gender][clothingName] &&
                                    whitelisted[gender][clothingName].indexOf(clothingID) > -1
                                )
                            ||
                                (
                                    hiddenItems[gender][clothingName] 
                                    && hiddenItems[gender][clothingName].indexOf(clothingID) > -1
                                )
                            )
                            {
                                isNotValid = true
