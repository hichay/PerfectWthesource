...
-- currentTattoos [[collectionHash, tatHash], [collectionHash, tatHash]]
-- loop tattooHashList [category] find [tatHash, collectionHash]
...
function Save(save, close, newFadeStyle)
    if save then
        ...
        if not startingMenu or passedClothing then
            ...
            TriggerServerEvent("raid_clothes:set_tats", currentTattoos)
    ...
    TriggerEvent('pw-weapons:getAmmo')
...
AddEventHandler("pw-inventory:itemUsed", function(pItem, pInfo)
    if pItem ~= "customclothing" then return end
    local data = json.decode(json.decode(pInfo).data)
    LoadPed(data)
end)
...
function setTattoosAndFacial(pTattoos, pFadeStyle)
        ...
        for k, v in pairs(pTattoos) do
            for category in pairs(tattooHashList) do
                if k == category then
                    ...
                    if tattoo ~= nil then
                        currentTattoos[#currentTattoos + 1] = {tattoo[2], tattoo[1]}
                    end
                end
            end
        end
    ...
    for i = 1, #currentTattoos do
        ApplyPedOverlay(playerPed, currentTattoos[i][1], currentTattoos[i][2])
    end
end
...
RegisterUICallback("np-ui:raid_clothes:changeOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    TriggerEvent('hotel:outfit', { true, data.key[1] }, 3, data.key[2])
end)
...
AddEventHandler('raid_clothes:ListOutfits', function(skincheck, pIgnoreDistance)
    ...
    for i = 1, #skincheck do
        ...
        menuData[#menuData + 1] = {
            ...
            key = slot,
            children = {
                { title = "Change Outfit", action = "np-ui:raid_clothes:changeOutfit", key = { slot, pIgnoreDistance }},
                { title = "Delete Outfit", action = "np-ui:raid_clothes:deleteOutfit", key = { slot, pIgnoreDistance }},
            }
        }
    end
    if #menuData > 0 then
...
AddEventHandler("pw-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports["pw-interaction"]:showInteraction(("[M] %s"):format(currentZone.text))
...
RegisterNetEvent("pw-inventory:itemCheck", function(item)
  local model = chainModels[item]
  if not model then return end
  local qty = exports["pw-inventory"]:hasEnoughOfItem(item, 1)
...
local function hasChainAccess()
  ...
  local jobAccess = RPC.execute("IsEmployedAtBusiness", { character = { id = characterId }, business = { id = "jeweled_dragon" } })
  if not jobAccess then
    TriggerEvent("DoLongHudText", "They do not recognize you", 2)
    return false
  end
  local craftAccess = RPC.execute("np-business:hasPermission", "jeweled_dragon", "craft_access", characterId)
...
AddEventHandler("np-clothing:infuseChainWithGems", function()
  ...
  RPC.execute("np-clothing:infuseCGChain", cid)
end)
