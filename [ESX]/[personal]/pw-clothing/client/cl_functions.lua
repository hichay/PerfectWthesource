    ...
    for i = 1, #Config.ClothingRooms do
        ...
        if Room['IsGang'] then
            TargetAction = function()
                ...
                TriggerEvent('pw-clothing:client:getOutfits', Room['Job'], Grade)
            end
        else
            TargetAction = function()
    ...
    StoreCombo:onPlayerInOut(function(isPointInside, point, zone)
        if isPointInside then
            ...
            if zoneName == 'surgeon' then
                exports["pw-interaction"]:showInteraction(("[E] %s"):format("Plastic Surgery"))
            elseif zoneName == 'clothing' then
                exports["pw-interaction"]:showInteraction(("[E] %s"):format("Clothing Shop"))
            elseif zoneName == 'barber' then
                exports["pw-interaction"]:showInteraction(("[E] %s"):format("Barber"))
            end
        else
            inZone = false
    ...
    RoomCombo:onPlayerInOut(function(isPointInside, point, zone)
        if isPointInside then
           -- print(ESX.GetPlayerData().job.name)
            ...
            local ZoneId = tonumber(SplitStr(zone.name, "_")[2])
            local JobName = Config.ClothingRooms[ZoneId]['IsGang'] and job
            if (job == Config.ClothingRooms[ZoneId]['Job']) then
                zoneName = ZoneId
                inZone = true
                exports["pw-interaction"]:showInteraction(("[E] %s"):format("Clothing Shop"))
            end
        ...
        if inZone then
            ...
            elseif zoneName == 'barber' then
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    --TriggerEvent('table',ESX.GetPlayerData().job)
                    local Grade = Room['IsGang']  or ESX.GetPlayerData().job.grade
                    TriggerEvent('pw-clothing:client:getOutfits', Room['Job'], Grade)
                end
            end
        else
...
function SaveSkin(IsAdmin)
    -- Mercy.Functions.TriggerCallback('pw-clothing/server/pay-for-clothes', function(HasPaid)
    local HasPaid = RPC.execute("pw-clothing:server:pay-for-clothes", IsAdmin)
    if HasPaid then
        local Model = GetEntityModel(PlayerPedId())
        local Clothing = json.encode(Config.SkinData)
        TriggerServerEvent("pw-clothing:saveSkin", Model, Clothing, IsAdmin)
    end
end
