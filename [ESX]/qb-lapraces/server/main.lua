...
AddEventHandler('qb-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('qb-phone:client:RaceNotify', src, 'Bạn đã nhận được WR của '..RaceData.RaceName..' bị phá vỡ với một thời gian: '..SecondsToClock(BLap)..'!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
        ...
        MySQL.Sync.execute("UPDATE `lapraces` SET `records` = '"..json.encode(Races[RaceData.RaceId].Records).."' WHERE `raceid` = '"..RaceData.RaceId.."'", {})
        TriggerClientEvent('qb-phone:client:RaceNotify', src, 'Bạn đã nhận được WR của '..RaceData.RaceName..' đặt xuống với thời gian: '..SecondsToClock(BLap)..'!')
    end
...
AddEventHandler('qb-lapraces:server:CreateLapRace', function(RaceName)
    ...
    if IsNameAvailable(RaceName) then
        ...
        TriggerClientEvent('ESX:Notify', source, 'Đã có một cuộc đua với tên này.', "error")
    end
end)
...
AddEventHandler('qb-lapraces:server:JoinRace', function(RaceData)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('ESX:Notify', src, 'Bạn là người duy nhất trong cuộc đua. Cuộc đua kết thúc.', "info")
    ...
    if creatorsource ~= Player.source then
        TriggerClientEvent('qb-phone:client:RaceNotify', creatorsource, string.sub(esxChar.firstname, 1, 1)..'. '..esxChar.lastname..' tham gia cuộc đua!')
    end
end)
...
AddEventHandler('qb-lapraces:server:LeaveRace', function(RaceData)
    ...
    local creatorsource = ESX.GetPlayerFromIdentifier(AvailableRaces[AvailableKey].SetupSteam).source
    if creatorsource ~= Player.source then
        TriggerClientEvent('qb-phone:client:RaceNotify', creatorsource, string.sub(esxChar.firstname, 1, 1)..'. '..esxChar.lastname..' rời khỏi cuộc đua!')
    end
    local AmountOfRacers = 0
    for k, v in pairs(Races[RaceData.RaceId].Racers) do
        AmountOfRacers = AmountOfRacers + 1
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('ESX:Notify', src, 'Bạn là người duy nhất trong cuộc đua. Cuộc đua tạm kết thúc.', "info")
    ...
    if Races[RaceId] ~= nil then
        if not Races[RaceId].Waiting then
            if not Races[RaceId].Started then
                ...
                TriggerClientEvent('ESX:Notify', source, 'The race is already active.', "info")
            end
        else
            TriggerClientEvent('ESX:Notify', source, 'The race is already active.', "info")
        end
    else
        TriggerClientEvent('ESX:Notify', source, 'This race does not exist.', "info")
    end
end)
...
AddEventHandler('qb-lapraces:server:StartRace', function(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupSteam == MyPlayer.identifier then
            ...
            TriggerClientEvent('ESX:Notify', src, 'You are not the maker of the race.', "info")
        end
    else
        TriggerClientEvent('ESX:Notify', src, 'You are not in a race.', "info")
    end
end)
...
RegisterCommand("togglesetup", function(source, args)
    ...
    if not Config.RaceSetupAllowed then
        TriggerClientEvent('ESX:Notify', source, 'Không thể tạo thêm cuộc đua!', "error")
    else
        TriggerClientEvent('ESX:Notify', source, "Cuộc đua có thể được tạo lại!", "info")
    end
end)
...
RegisterCommand('cancelrace', function(source, args, user)
    ...
    if RaceName ~= nil then
        ...
        if Races[RaceId].Started then
            ...
            TriggerClientEvent('ESX:Notify', source, 'Cuộc đua này vẫn chưa bắt đầu.', "info")
        end
    end
end)
