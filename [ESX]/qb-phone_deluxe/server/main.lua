                                ...
                                if invoices[1] ~= nil then
                                    for k, v in pairs(invoices) do
                                        ...
                                        if Ply ~= nil then
                                            ...
                                            ExecuteSql(true, "SELECT * FROM `users` WHERE `identifier` = '"..v.author_identifier.."'", function(res)
                                                if res[1] ~= nil then
...
RegisterServerEvent('qb-phone:server:sellVehicle')
AddEventHandler('qb-phone:server:sellVehicle', function(plate, price)
...
RegisterServerEvent('qb-phone:server:buy:chosen:vehicle')
AddEventHandler('qb-phone:server:buy:chosen:vehicle', function(VehiclePlate, CitizenId, SellPrice)
    local src = source
    ...
    if Player.getAccount('bank').money >= SellPrice then
        ...
        if TargetPlayer ~= nil then
           ...
           TriggerClientEvent('qb-phone:client:send:email:bought:vehicle', src, VehiclePlate) 
           TriggerClientEvent('qb-phone:client:send:email:sold:vehicle', TargetPlayer.source, SellPrice, VehiclePlate) 
           TriggerEvent('ESX:Notify', src, "Xe có biển số là '" .. VehiclePlate .. "' đã được mua thành công $'" .. SellPrice .. "'", "success")
        else
            ExecuteSql(false, 'SELECT `accounts` FROM user WHERE identifier ="'..CitizenId..'"',function(result)
                if result ~= nil then
                 local NewMoneyTable = {}
                 ...
                 local MailData = {
                    sender = "Autoscout24",
                    subject = "Thông tin bán",
                    message = "Khách hàng thân mến,<br/><br/>Bạn sẽ nhận được một email về việc đăng bán xe với thông tin.<br><br>Biển số: <strong>" ..VehiclePlate.. "</strong> <br>Giá bán: <strong>$"..SellPrice.. '</strong><br><br>Xe của bạn đã được bán thành công và số tiền đã được chuyển vào tài khoản ngân hàng.<br><br>Thân trọng,<br>Autoscout24',
                    button = {}
                 }
                 TriggerEvent("qb-phone:server:sendNewEventMail", CitizenId, MailData)
                 Player.removeAccountMoney('bank', SellPrice)	
                 ExecuteSql(false, "UPDATE owned_vehicles SET `owner` = '" .. Player.identifier .. "', `state` = '"..GarageData.."', `forSale` = '0', `salePrice` = '0' WHERE `plate` = '" ..VehiclePlate.. "'")
                 ExecuteSql(false, "UPDATE user SET `accounts` = '" ..json.encode(NewMoneyTable).. "' WHERE `identifier` = '" ..CitizenId.. "'")
                 TriggerClientEvent('fw-phone:client:send:email:bought:vehicle', src, VehiclePlate)
                 TriggerClientEvent('ESX:Notify', src, "Xe có biển số là '" .. VehiclePlate .. "' đã được mua thành công €'" .. SellPrice .. "'", "success")
                end
            end)
        end
    else
...
ESX.RegisterServerCallback('qb-phone:server:load:autoscout', function(source, cb)
...
AddEventHandler('qb-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    ...
    if Target ~= nil then
		...
		--TriggerClientEvent('cs-video-call:custom:setCallee', Target.source, source)
		--TriggerClientEvent('cs-video-call:custom:setCallee', source, Target.source)
    ...
    ExecuteSql(false, "SELECT * FROM `users` WHERE `iban`='"..iban.."'", function(result)
        if result[1] ~= nil then
            ...
            if recieverSteam ~= nil then
                ...
                if true then
                    TriggerClientEvent('qb-phone:client:TransferMoney', recieverSteam.source, amount, infobank)
					--local text = 'Tiền đã được chuyển chuyển vào tài khoản của bạn: $' .. amount .. ', IBAN người gửi: ' .. result[1].iban ..''
					--TriggerClientEvent("qb-phone:client:BankNotify",text)	
                    ExecuteSql(false, "SELECT * FROM `users` WHERE `identifier`='"..ESX.GetPlayerFromId(src).identifier.."'", function(result)
						--TriggerClientEvent("ESX:Notify",sender,"Chuyển tiền thành công","success")
						--TriggerClientEvent("ESX:Notify",recieverSteam,text,"info")
                        -- TriggerClientEvent('rp_notify:client:SendAlert', sender.source, { type = 'inform', text = 'Para transferi başarılı!'})
                        -- TriggerClientEvent('rp_notify:client:SendAlert',  recieverSteam.source, { type = 'inform', text = 'Tiền đã đưởng chuyển vào tài khoản của bạn: $' .. amount .. ', IBAN người gửi: ' .. result[1].iban .. ''})
                    end)
                end
            ...
            TriggerClientEvent("ESX:Notify",src,"IBAN không tồn tại","error")
        end
    end)
end)
...
ESX.RegisterServerCallback('qb-phone:server:HasPhone', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
	--cb(true)
	--exports.pw-inventory:hasEnoughOfItem("phone",1,false)
	--exports["pw-inventory"]['hasEnoughOfItem']("phone",1,false)
	--exports["pw-inventory"]:hasEnoughOfItem("phone",1,false)
    if exports.inventory:hasEnoughOfItem("phone",1,false) then
