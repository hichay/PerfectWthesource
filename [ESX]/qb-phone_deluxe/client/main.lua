...
Citizen.CreateThread(function()
    ...
    TriggerEvent("chat:removeSuggestion", "/phone")			
    --while true do
                ...
                ESX.TriggerServerCallback('qb-phone:server:GetGarageVehicles', function(vehicles)
                    ...
                    if vehicles ~= nil then
                        for k, v in pairs(vehicles) do
                            vehicles[k].fullname = GetLabelText(Citizen.InvokeNative(0xF7AF4F159FF99F97 , v.model, Citizen.ResultAsString()))
							--GetLabelText(Citizen.InvokeNative(0xF7AF4F159FF99F97, vehicleModel, Citizen.ResultAsString()))
                        end
                    end
...
RegisterNUICallback('LoadAutoscout', function()
    ESX.TriggerServerCallback('qb-phone:server:load:autoscout', function(Data)
        SendNUIMessage({
            action = "RefreshAutoscout",
            Cars = Data
        })
...
while takePhoto do
    ...
    if IsControlJustPressed(1, 27) then -- Toogle Mode
     ...
     exports['screenshot-basic']:requestScreenshotUpload('https://discord.com/api/webhooks/871842880799510578/B9ua40_BMwQ1wSZ2gMRUDTx8DgZKWY5_zE2pYNWkfXB2KElaLQatB70nmMdYAxIcnuI6', 'files[]', function(data2)
        DestroyMobilePhone()
...
AddEventHandler('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    if PhoneData.isOpen then
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Ngân Hàng", text = "Bạn vừa nhận được "..amount.." $!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = newmoney })
    else
        SendNUIMessage({ action = "Notification", NotifyData = { title = "Ngân Hàng", content = "Bạn vừa nhận được "..amount.." $!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)
...
RegisterCommand('test', function()
    ...
    TriggerEvent('qb-phone:client:addPoliceAlert', { sender = "FireLeaf", title = "Test!", description = "https://discord.gg/BYK9XEP2sz"})		 
end)