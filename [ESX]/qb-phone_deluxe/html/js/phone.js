...
MI.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        ...
        $("#total-contacts").text(TotalContacts+ " Danh bạ");
    } else {
        $("#total-contacts").text("0 Danh bạ");
    }
};
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!MI.Phone.Data.CallActive) {
            ...
            MI.Phone.Animations.TopSlideUp('.'+MI.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "Số đang gọi bạn: "+CallData.name
                if (AnonymousCall) {
                    Label = "Bạn đang gọi ở chế độ ẩn danh."
                }
