...
$(document).on('click', '.bank-app-account', function(e){
    ...
    MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("BANK_TITLE"), "Đã copy IBAN", "#badc58", 1750);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
        if (Number(amountData) >= Number(amount)) {
            $.post('http://qb-phone_deluxe/TransferMoney', JSON.stringify({
                ...
                if (data.CanTransfer) {
                    ...
                    MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("BANK_TITLE"), "Bạn đã chuyển &euro; "+amount+",!", "#badc58", 1500);
                } else {
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://qb-phone_deluxe/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("BANK_TITLE"), "Bạn đã chi trả &euro;"+InvoiceData.amount+" !", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    MI.Phone.Notifications.Add("fas fa-university", MI.Phone.Functions.Lang("BANK_TITLE"), "Bạn đã huỷ thanh toán &euro;"+InvoiceData.amount+"!", "#badc58", 1500);
});
...
MI.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
        ...
        $.each(invoices, function(i, invoice){
            var Elem = '<div class="bank-app-invoice" id="invoiceid-'+i+'"> <div class="bank-app-invoice-title">'+ invoice.label +' 			 	</div><br><div class="bank-app-invoice-amount">$ '+invoice.amount+'</div></br>              <span style="font-size: 2vh; color: gray; position:absolute:;"> Người gửi: '+invoice.number+'</span> <div class="bank-app-invoice-buttons"> <i class="fas fa-check-circle pay-invoice"></i></div> ';
