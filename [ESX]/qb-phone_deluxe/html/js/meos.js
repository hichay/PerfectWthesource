...
SetupMeosHome = function() {
    $("#meos-app-name").html("Xin chào: " + MI.Phone.Data.PlayerData.charinfo.firstname + " " + MI.Phone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.person-search-result', function(e){
	...
	var job = "abc";
	if (ClickedPersonData.job == "unemployed") {
        job  = "Tự do";
    }
	else if(ClickedPersonData.job == "police"){
		job = "Cảnh sát"
	}
	else if(ClickedPersonData.job == "ambulance"){
		job = "Bác sĩ"
	}

    var OpenElement = '<div class="person-search-result-name">Name: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">Phone: '+ClickedPersonData.phone+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Birthday: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-gender">Sex: '+Gender+'</div> &nbsp; <div class="person-search-result-warned">Nghề nghiệp: '+job+'</div> <div class="person-search-result-driverslicense">Drivers license: '+HasLicense+'</div>';
        ...
        $.post('http://qb-phone_deluxe/FetchVehicleResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, vehicle) {
                    ...
                    var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">'+vehicle.label+'</div> <div class="vehicle-search-result-plate">Biển số: '+vehicle.plate+'</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Chủ: '+vehicle.owner+'</div> &nbsp; <div class="vehicle-search-result-apk">Tình trạng: '+vehicle.state+'</div>  </div>'
...
$(document).on('click', '.scan-search-vehicle', function(e){
    ...
    $.post('http://qb-phone_deluxe/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            ...
            var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">'+vehicle.label+'</div> <div class="vehicle-search-result-plate">Biển số: '+vehicle.plate+'</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Chủ xe: '+vehicle.owner+'</div> &nbsp; <div class="vehicle-search-result-apk">Tình trạng: '+vehicle.state+'</div> </div>'
            $(".vehicle-search-results").append(VehicleElement);
        } else {
...
$(document).on('click', '.meos-clear-alerts', function(e){
    $(".meos-alerts").html("");
    $(".meos-recent-alerts").html('<div class="meos-recent-alert"> <span class="meos-recent-alert-title">Bạn không có thông báo nào cả!</span></div>');
