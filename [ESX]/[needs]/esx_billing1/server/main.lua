...
RegisterServerEvent('esx_billingmk:sendBill')
AddEventHandler('esx_billingmk:sendBill', function(playerId, sharedAccountName, label, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xTarget = ESX.GetPlayerFromIdCard(playerId)
					...
					if xPlayer.getMoney() >= amount then
						MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
							...
							if rowsChanged == 1 then
								...
								TriggerEvent("pw-bossmenu:server:addAccountMoney", result[1].target, amount)
