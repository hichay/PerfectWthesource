...
function discord_messagewithdraw(title)
...
function discord_messagedespoit(title)
	local username = 'Voucher'
	local webhook = ''
	if Config.DiscordWebhook then
		webhook = Config.DiscordWebhook
...
AddEventHandler('esx_society:withdrawMoney', function(society, amount)
	...
	if xPlayer.job.name == society.name or (xPlayer.gang.name == society.name) then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				local title = string.format(Config.WithdrawMoney, GetPlayerName(source), society.name, ESX.Math.GroupDigits(amount))
				discord_messagewithdraw(title)
			else
...
AddEventHandler('esx_society:depositMoney', function(society, amount)
	...
	if xPlayer.job.name == society.name or (xPlayer.gang.name == society.name) then
		if amount > 0 and xPlayer.getMoney() >= amount then
			...
			local title = string.format(Config.DepositMoney, GetPlayerName(source), society.name, ESX.Math.GroupDigits(amount))
			discord_messagedespoit(title)
		else
			xPlayer.showNotification(_U('invalid_amount'))
		end
	else
...
ESX.RegisterServerCallback('esx_society:getEmployees2', function(source, cb, society)
	if Config.EnableESXIdentity then
		...
		MySQL.Async.fetchAll('SELECT firstname, lastname, identifier, gang, gang_grade FROM users WHERE gang = @gang ORDER BY gang_grade DESC', {
			['@gang'] = society
		}, function (results)
			local employees = {}
			...
			for i=1, #results, 1 do
				table.insert(employees, {
					...
					gang = {
						name        = results[i].gang,
						label       = Jobs[results[i].gang].label,
						grade       = results[i].gang_grade,
						grade_name  = Jobs[results[i].gang].grades[tostring(results[i].gang_grade)].name,
						grade_label = Jobs[results[i].gang].grades[tostring(results[i].gang_grade)].label
					}
				})
			end
		...
		MySQL.Async.fetchAll('SELECT name, identifier, gang, gang_grade FROM users WHERE gang = @gang ORDER BY gang_grade DESC', {
			['@gang'] = society
		}, function (result)
			local employees = {}
			...
			for i=1, #result, 1 do
				table.insert(employees, {
					...
					gang = {
						name        = result[i].gang,
						label       = Jobs[result[i].gang].label,
						grade       = result[i].gang_grade,
						grade_name  = Jobs[result[i].gang].grades[tostring(result[i].gang_grade)].name,
						grade_label = Jobs[result[i].gang].grades[tostring(result[i].gang_grade)].label
					}
				})
			end
...
ESX.RegisterServerCallback('esx_society:getGang', function(source, cb, society)
	local gang    = json.decode(json.encode(Jobs[society]))
	local grades = {}
...
ESX.RegisterServerCallback('esx_societymk:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_society:setGang', function(source, cb, identifier, gang, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBoss = xPlayer.gang.grade_name == 'boss'
	...
	if isBoss then
		...
		if xTarget then
			...
			elseif type == 'promote' then
				xTarget.showNotification(_U('you_have_been_promoted'))
			elseif type == 'fire' then
				xTarget.showNotification(_U('you_have_been_fired', xTarget.getGang().label))
			end
		...
		print(('esx_society: %s attempted to setGang'):format(xPlayer.identifier))
		cb()
	end
end)
...
ESX.RegisterServerCallback('esx_societymk:getOnlinePlayers', function(source, cb)
	local xPlayers = ESX.GetPlayers()
	local players  = {}
...
function isPlayerBoss(playerId, job)
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'boss' or (xPlayer.gang.name == job and xPlayer.gang.grade_name == 'boss') then
		return true
	else
