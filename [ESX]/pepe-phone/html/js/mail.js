...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-lastsync").html("Last sync: "+MessageTime);
...
var MonthFormatting = ["Jan", "Feb", "March", "April", "May", "June", "July", "Aug", "Sept", "Oct", "Nov", "Dec"];
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        QB.Phone.Notifications.Add("fas fa-ad", "Advertisements", "You can't post an empty advertisement!", "#ff8f1a", 2000);
    }
});
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">No advertisements posted yet!</span></div>';
        $(".advert-list").append(element);
    }
}