...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('http://pepe-phone/ClearRecentAlerts');
                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (QB.Phone.Data.AnonymousCall) {
                            QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Bạn đã bắt đầu một cuộc gọi ẩn danh!");
                        }
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Bạn đã tham gia một cuộc gọi!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Người này đang bận!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Người này không có sẵn!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Bạn không thể gọi số của mình!");
        }
    });
});
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            $.post('http://pepe-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "Bạn không thể gửi tin cho bản thân mình..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://pepe-phone/EditContact', JSON.stringify({
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Contact Edit", "Điền toàn bộ thông tin!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://pepe-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://pepe-phone/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://pepe-phone/RemoveSuggestion', JSON.stringify({
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Add contact", "fill in all fields!");
    }
});
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('http://pepe-phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('http://pepe-phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();
    
    $.post('http://pepe-phone/CancelOngoingCall');
});
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "Đang có cuộc gọi từ "+CallData.name
                if (AnonymousCall) {
                    Label = "Bạn đang bị gọi bởi số ẩn danh"
                }
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('http://pepe-phone/AnswerCall');
});
