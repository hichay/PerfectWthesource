...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    $.post('http://pepe-phone/ClearAlerts', JSON.stringify({
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('http://pepe-phone/SendMessage', JSON.stringify({
        ...
        QB.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "You can't send an empty message!", "#25D366", 1750);
    }
});
...
$(document).on('click', '#send-tikkie', function(e){
    e.preventDefault();
    $.post('http://pepe-phone/SendTikkie', JSON.stringify({
        ChatNumber: OpenedChatData.number,
        ChatDate: GetCurrentDateKey(),
        ChatMessage: "Hey I sent you a paiment request, look in your bank app for the requested payment amount.",
        ChatTime: FormatMessageTime(),
        ChatType: "message",
    }));
});
...
//         if (OpenedChatPicture == null) {
//             $.post('http://pepe-phone/GetProfilePicture', JSON.stringify({
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('http://pepe-phone/SharedLocation', JSON.stringify({
