...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Bank account no. copied!", "#badc58", 1750);
});
...
QB.Phone.Functions.DoBankOpen = function(data) {
    ...
    $(".bank-app-account-balance").html("Bank: "+data.bank.toFixed());
    $(".bank-app-account-balance").data('balance', data.bank.toFixed());
	$(".bank-app-accountmo-balance").html("Tiền mặt: "+data.money.toFixed());
    $(".bank-app-accountmo-balance").data('balance', data.money.toFixed());
			...
			if (amount > 0) {
				$.post('http://pepe-phone/TransferMoney', JSON.stringify({
					...
					if (data.CanTransfer) {
						...
						QB.Phone.Notifications.Add("fas fa-university", "Bank", "Không có đủ tiền!", "#badc58", 1500);
					}
				})
				QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
			} else {
				QB.Phone.Notifications.Add("fas fa-university", "Bank", "Số tiền không thể nhỏ hơn 0", "#badc58", 1500);
			 }
        } else {
            QB.Phone.Notifications.Add("fas fa-university", "Bank", "Không có đủ tiền!", "#badc58", 1500);
        }
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "Bank", "Hãy ghi đủ thông tin!", "#badc58", 1750);
    }
});
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://pepe-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Bank", "You have paid &dollar;"+InvoiceData.amount+" !", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Bank", "You declined the request.", "#badc58", 1500);
 } else {
    QB.Phone.Notifications.Add("fas fa-university", "Bank", "You cannot refuse this request.", "#badc58", 1500);
 }
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", " Bank", "Liên lạc này không có IBAN!", "#badc58", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});