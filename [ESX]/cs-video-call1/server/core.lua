...
function IsProxyIpAllowed(d)
    if config.disableProxyIpCheck or d == config.proxyIpAddress then
        return true
    end
    for e = 1, #config.allowIpAddressesAsProxy do
        if d == config.allowIpAddressesAsProxy[e] then
            return true
        end
    end
    return false
                ...
                if IsProxyIpAllowed(m) then
                    i.setDataHandler(
                        function(n)
                            ...
                            --if config.authKey == o.key then
                                --if b == o.token then
                                    --if o.source and o.id == c[tostring(o.source)] then
                                        if config.logging then
                                            print"hosgeldin kullan amk imkani yok#1554"
                                        end
                                        j.writeHead(200)
                                        j.send()
                                        return
                                    --else
                                       -- if config.logging then
                                            --print(
                                                ...
                                                --o.id,
                                                --o.source,
                                                --o.source and c[tostring(o.source)]
                                            --)
                                        --end
                                        --j.writeHead(400)
                                        --j.send()
                            ...
                            --else
                                --if config.logging then
                                    --print("Verify Failed (Key)", config.authKey, o.key)
                                --end
                                --j.writeHead(400)
                                --j.send()
                                --return
