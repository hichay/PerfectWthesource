...
config = {
    ...
    -- Alternatively you can disable the proxy IP check altogether if your proxy server communicates with your server via a reverse proxy that assigns a dynamic IP address.
    -- If your players connect to your server or the proxy server communicates with your server via a reverse proxy that assigns a dynamic IP address you can use an authentication key instead.
    -- The exact same authentication key needs to be set in the proxy server's config so they match up.
    allowIpAddressesAsProxy = {},
    disableProxyIpCheck = false,
    authKey = "obfuscatorunu gotten sikim amk bytecode mu kullaniyosun cidden",
